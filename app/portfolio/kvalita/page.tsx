...
import { unitConditionPriority } from "@/lib/portfolio/unit-condition-execution";
...
export default async function PortfolioQualityPage({ searchParams }: { searchParams: Promise<{ ok?: string; error?: string; properties?: string }> }) {
  ...
  const [latest, openExecutions] = unitIds.length ? await Promise.all([prisma.unitConditionAssessment.findMany({
    ...
    include: { createdBy: { select: { name: true } }, execution: { include: { task: { select: { id: true, status: true } }, propertyCost: { select: { id: true } }, budgetLine: { select: { id: true, year: true } } } } },
  ...
  const rows = units.map((unit) => { const assessment = byUnit.get(unit.id); return { unit, assessment, priority: assessment ? unitConditionPriority(assessment) : null }; }).sort((a, b) => (b.priority?.score ?? -1) - (a.priority?.score ?? -1) || a.unit.property.name.localeCompare(b.unit.property.name, "cs") || a.unit.label.localeCompare(b.unit.label, "cs"));
  ...
  return <Shell user={user}><div className="page portfolio-quality-page">
    ...
    <section className="card portfolio-table-card"><div className="table-toolbar"><div><h2>Prioritní fronta obnovy</h2><p>Pořadí kombinuje stav, naléhavost, fázi plánu a prošlý termín. Schválený plán lze jednou převést do úkolu, plánovaného nákladu a ročního rozpočtu.</p></div></div><div className="table-wrap"><table><thead><tr><th>Priorita</th><th>Nemovitost / jednotka</th><th>Kvalita</th><th>Naléhavost</th><th>Plán</th><th>Odhad CAPEX</th><th>Hodnoceno</th><th></th></tr></thead><tbody>{rows.length ? rows.map(({ unit, assessment, priority }) => {
      ...
      return <tr key={unit.id}><td>{priority ? <span className={`status ${priority.tone}`}>{priority.label}</span> : "—"}</td><td><Link className="entity-link" href={`/nemovitosti/${unit.property.id}/jednotky/${unit.id}#kvalita`}>{unit.property.name} · {unit.label}</Link><span className="owner-sub">{unit.areaM2 ? `${unit.areaM2.toLocaleString("cs-CZ")} m²` : "Plocha chybí"}{!unit.property.active ? " · archivováno" : ""}</span></td><td>{assessment ? <span className={`rating-badge rating-${assessment.rating.slice(0, 1).toLowerCase()}`}>{unitConditionRatings[assessment.rating]}</span> : <span className="status warn">Nehodnoceno</span>}</td><td>{assessment ? unitConditionUrgencies[assessment.investmentUrgency] : "—"}</td><td>{assessment ? <><span className={`status ${assessment.planStatus === "COMPLETED" ? "ok" : assessment.planStatus === "IN_PROGRESS" ? "warn" : ""}`}>{unitConditionPlanStatuses[assessment.planStatus]}</span><span className="owner-sub">{assessment.targetDate ? `Cíl ${date(assessment.targetDate)}` : "Bez termínu"}</span>{assessment.execution && <span className="owner-sub">Převedeno do realizace</span>}</> : "—"}</td><td className="money">{assessment ? money(assessment.estimatedCapexCents) : "—"}</td><td>{assessment ? <><strong>{date(assessment.assessedAt)}</strong><span className="owner-sub">{assessment.createdBy.name}</span></> : "—"}</td><td><div className="quality-row-actions">{assessment?.execution ? <div className="execution-links"><Link href={`/ukoly/${assessment.execution.task.id}`}>Úkol</Link><Link href={`/nemovitosti/${unit.property.id}/naklady/${assessment.execution.propertyCost.id}`}>CAPEX</Link><Link href={`/nemovitosti/${unit.property.id}/finance?financeYear=${assessment.execution.budgetLine.year}#rozpocet`}>Rozpočet</Link></div> : hasOpenExecution ? <span className="status warn">Otevřená realizace</span> : canManage && assessment?.planStatus === "APPROVED" ? <details className="condition-execution"><summary>Převést plán</summary><UnitConditionExecutionForm propertyId={unit.property.id} unitId={unit.id} assessmentId={assessment.id} unitLabel={unit.label} returnTo={returnTo}/></details> : null}{canManage ? <details className="distribution-assessment condition-assessment"><summary>Nový snapshot</summary><UnitConditionAssessmentForm propertyId={unit.property.id} unitId={unit.id} returnTo={returnTo} assessment={assessment}/></details> : <Link className="table-link" href={`/nemovitosti/${unit.property.id}/jednotky/${unit.id}#kvalita`}>Detail</Link>}</div></td></tr>;
