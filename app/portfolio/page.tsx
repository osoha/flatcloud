...
export default async function Portfolio({ searchParams }: { searchParams: Promise<{ ok?: string; error?: string; properties?: string; propertyId?: string }> }) {
  ...
  const comparisonCharges = await prisma.charge.findMany({ where: { leaseId: { in: visibleLeaseIds }, active: true }, select: {
    period: true, active: true, amountCents: true,
  ...
  // Do not pair a current total with a comparison using a different cutoff or ledger amount.
  const collectionTrend = comparison?.current.paid === paid && comparison.current.expected === expected ? comparison : null;
  ...
  return <Shell user={user}><div className="page v21-portfolio"><div className="page-title"><div><PageHeading>Portfolio</PageHeading><p>{portfolioSelectionLabel(selection,properties.length,availableProperties.length,availableProperties.filter((property)=>property.active).length)} · období {period}.</p></div><div className="action-row"><PortfolioScopePicker availableProperties={availableProperties.map(({id,name,address,city,active,owner,communicationOwner,flatcloudConsolidationBasisPoints})=>({id,name,address,city,active,ownerId:communicationOwner?.id||owner.id,ownerName:communicationOwner?.name||owner.name,scopeKind:flatcloudConsolidationBasisPoints==null?"UNCLASSIFIED" as const:flatcloudConsolidationBasisPoints>0?"FLATCLOUD" as const:"EXTERNAL" as const}))} selection={selection.mode==="ALL"?selection:{mode:"SELECTED",propertyIds:allowedSelection}}/></div></div><Flash ok={query.ok} error={query.error}/>
    <div className="stat-grid v21-stat-grid"><Kpi href={`/reporty?view=collections${scopeQuery}`} icon={<CheckCircle2/>} label="Inkaso" value={expected?`${Math.round(paid/expected*100)} %`:"—"} note={`${money(paid)} z ${money(expected)}`} tone="green" trend={<KpiTrend comparison={collectionTrend}/>}/><Kpi href={`/reporty?view=collections${scopeQuery}`} icon={<WalletCards/>} label="Dluh" value={money(debt)} note="po splatnosti" tone={debt > 0 ? "red" : "green"} bad={debt>0}/><Kpi href={`/ukoly?${scopeQuery.slice(1)}`} icon={<ListChecks/>} label="Úkoly" value={String(taskCount)} note="otevřených případů" tone="orange" bad={tasks.some(t=>t.priority==="URGENT")}/><Kpi href={`/revize?${scopeQuery.slice(1)}`} icon={<ClipboardCheck/>} label="Revize" value={String(revisionCount)} note={`${overdueRevisionCount} po termínu`} tone="purple" bad={overdueRevisionCount>0}/><Kpi href={`/smlouvy/upozorneni?${scopeQuery.slice(1)}`} icon={<CalendarCheck2/>} label="Smlouvy" value={String(contractAlerts.length)} note={`${expiryCount} expirace · ${anniversaryCount} výročí`} tone="blue"/>{user.role==="SUPER_ADMIN"&&<Kpi href={`/platby/nesparovane?${scopeQuery.slice(1)}`} icon={<AlertCircle/>} label="Nespárované" value={String(unmatchedCount)} note="plateb k řešení" tone="red" bad={unmatchedCount>0}/>}</div>
    <div className="card portfolio-table-card" id="nemovitosti"><div className="table-toolbar"><div><h2>Nemovitosti</h2><p>Kliknutím otevřete provozní dashboard objektu.</p></div></div><div className="table-wrap"><table><thead><tr><th>Nemovitost</th><th>Komunikační vlastník / SVJ</th><th>KPI FlatCloud</th><th>Bankovní e-mail</th><th>Předpis</th><th>Dluh</th><th>Inkaso</th><th>Stav</th><th>Oblíbené</th></tr></thead><tbody>{rows.length?<>{renderPropertyRows(activeRows, photos.properties, appearances)}{inactiveRows.length>0&&<tr className="archive-section-row"><td colSpan={9}>Neaktivní / archivované</td></tr>}{renderPropertyRows(inactiveRows, photos.properties, appearances, true)}</>:<tr><td colSpan={9} className="table-empty">Zatím nejsou evidované nemovitosti.</td></tr>}</tbody></table></div><details className="method-tip"><summary>Oblíbené objekty a vlastní vzhled</summary><p>Hvězdičkou přidáte objekt mezi oblíbené a posunete jej nahoru v seznamu. Barvu karty a fotografii či obecný avatar vyberete ve Vzhledu objektu. Nastavení se ukládá k vašemu účtu. Barva je osobní pomůcka, nemění finanční stav; volbou „Bez zvýraznění“ ji odstraníte.</p></details></div>
