...
import { AlertCircle, CalendarCheck2, CheckCircle2, ClipboardCheck, ListChecks, WalletCards, Wrench } from "lucide-react";
...
export default async function Portfolio({ searchParams }: { searchParams: Promise<{ ok?: string; error?: string; properties?: string; propertyId?: string }> }) {
  ...
  return <Shell user={user}><div className="page v21-portfolio"><div className="page-title"><div><h1>Portfolio</h1><p>{portfolioSelectionLabel(selection,properties.length,availableProperties.length,availableProperties.filter((property)=>property.active).length)} · období {period}.</p>{fullAccess&&<span className="scope-context-badge">Provozní cockpit · napříč vlastníky</span>}</div><div className="action-row"><Link className="secondary" href={`/portfolio/kvalita${selectionValue === null ? "" : `?properties=${encodeURIComponent(allowedSelection.join(","))}`}`}><Wrench size={15}/> Kvalita a CAPEX</Link><PortfolioScopePicker availableProperties={availableProperties.map(({id,name,address,city,active,owner,communicationOwner,flatcloudConsolidationBasisPoints})=>({id,name,address,city,active,ownerId:communicationOwner?.id||owner.id,ownerName:communicationOwner?.name||owner.name,scopeKind:flatcloudConsolidationBasisPoints==null?"UNCLASSIFIED" as const:flatcloudConsolidationBasisPoints>0?"FLATCLOUD" as const:"EXTERNAL" as const}))} selection={selection.mode==="ALL"?selection:{mode:"SELECTED",propertyIds:allowedSelection}}/></div></div><Flash ok={query.ok} error={query.error}/>
    ...
    <div className="card portfolio-table-card" id="nemovitosti"><div className="table-toolbar"><div><h2>Nemovitosti</h2><p>Kliknutím otevřete provozní dashboard objektu.</p></div></div><div className="table-wrap"><table><thead><tr><th>Nemovitost</th><th>Komunikační vlastník / SVJ</th><th>KPI FlatCloud</th><th>Bankovní e-mail</th><th>Předpis</th><th>Dluh</th><th>Inkaso</th><th>Stav</th></tr></thead><tbody>{rows.length?<>{renderPropertyRows(activeRows)}{inactiveRows.length>0&&<tr className="archive-section-row"><td colSpan={8}>Neaktivní / archivované</td></tr>}{renderPropertyRows(inactiveRows,true)}</>:<tr><td colSpan={8} className="table-empty">Zatím nejsou evidované nemovitosti.</td></tr>}</tbody></table></div></div>
  </div></Shell>;
}
