...
export default async function Portfolio({ searchParams }: { searchParams: Promise<{ ok?: string; error?: string }> }) {
  ...
  const [properties, query] = await Promise.all([accessibleProperties(user, { includeInactive: true }), searchParams]);
  ...
  const unmatchedCount = user.role === "SUPER_ADMIN" ? (await Promise.all([
    ...
    prisma.inboxPayment.count({ where: { status: { in: ["RECEIVED", "UNMATCHED", "ERROR"] }, OR: [{ propertyId: null }, { propertyId: { in: propertyIds } }] } }),
  ...
  return <Shell user={user}><div className="page v21-portfolio"><div className="page-title"><div><h1>Portfolio</h1><p>Co vyžaduje pozornost napříč dostupnými nemovitostmi · období {period}.</p></div></div><Flash ok={query.ok} error={query.error}/>
