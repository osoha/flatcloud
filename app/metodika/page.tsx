...
export default async function MethodologyPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  return <Shell user={user}><div className="page methodology-page">
    ...
    {glossary.length>0&&<section className="methodology-section" id="slovnik"><div className="methodology-section-title"><Library size={18}/><h2>Slovník pojmů</h2></div><p className="methodology-section-copy">Jednotné významy pojmů používaných v aplikaci, reportech a rozhodovacích podkladech.</p><div className="methodology-glossary-grid">{glossary.map((term)=>{const chapter=methodologyChapters.find((item)=>item.slug===term.chapterSlug);return <article className="card methodology-term" key={term.term}><h3>{term.term}</h3><p>{term.definition}</p><small>Také: {term.aliases.join(", ")}</small>{chapter&&<Link href={`/metodika#${chapter.slug}`}>Související kapitola: {chapter.title}</Link>}</article>})}</div></section>}
