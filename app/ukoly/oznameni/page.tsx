import { AnnouncementReadMarker } from "@/components/AnnouncementReadMarker";
...
export default async function AnnouncementsPage({ searchParams }: { searchParams: Promise<{ view?: string; ok?: string; error?: string }> }) {
  ...
  return <Shell user={user}><div className="page announcements-page"><div className="page-title"><div><PageHeading>Oznámení</PageHeading><p>Provozní a legislativní sdělení určená vašemu účtu, roli nebo spravovaným nemovitostem.</p></div></div><TaskSectionNav user={user} active="announcements"/><Flash ok={query.ok} error={query.error}/><div className="task-status-tabs"><Link className={!hidden?"active":""} href="/ukoly/oznameni">Aktivní</Link><Link className={hidden?"active":""} href="/ukoly/oznameni?view=hidden">Skrytá</Link></div><div className="announcement-list">{announcements.length?announcements.map(item=><article id={`oznameni-${item.id}`} className={`card announcement-card severity-${item.severity.toLowerCase()}`} key={item.id}><div className="card-head"><div><span className={`status ${item.severity==="CRITICAL"?"bad":item.severity==="IMPORTANT"?"warn":"ok"}`}>{labels[item.severity]}</span><h2>{item.title}</h2></div><small>{item.expiresAt?`Platí do ${date(item.expiresAt)}`:"Bez exspirace"}</small></div><p>{item.body}</p>{!item.userStates[0]?.readAt && <AnnouncementReadMarker id={item.id}/> }<form action={`/api/announcements/${item.id}/state`} method="post"><input type="hidden" name="action" value={hidden?"restore":"dismiss"}/><input type="hidden" name="returnTo" value={hidden?"/ukoly/oznameni?view=hidden":"/ukoly/oznameni"}/><button className="secondary" type="submit">{hidden?"Vrátit mezi aktivní":"Skrýt"}</button></form></article>):<div className="card empty-state"><h2>{hidden?"Žádná skrytá oznámení":"Žádná aktivní oznámení"}</h2><p>{hidden?"Všechna sdělení se zobrazují standardně.":"Aktuálně pro vás není publikováno žádné sdělení."}</p></div>}</div></div></Shell>;
}
