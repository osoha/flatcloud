...
export default async function TaskDetail({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{ok?:string;error?:string}>}){
  ...
  const task=await prisma.task.findUnique({where:{id},include:{property:true,unit:true,lease:{include:{charges:{where:{active:true},include:{allocations:true,securityDepositOffsets:true,creditApplications:true}}}},tenant:true,assignee:true,createdBy:true,conditionPlanExecution:true,costs:{select:{id:true,title:true}},entries:{where:taskEntryVisibilityWhere(user),include:{author:true},orderBy:{createdAt:"desc"}}}});
  if(!task)notFound();
  ...
  return <Shell user={user} taskPropertyId={task.propertyId} taskLeaseId={task.leaseId||undefined}><div className="page task-case-page">
    ...
    <div className="page-title case-title"><div><PageHeading>{task.title}</PageHeading><div className="case-context"><Link href={`/nemovitosti/${task.propertyId}/prehled`}><Home size={13}/>{task.property.name}</Link>{task.unit&&<Link href={`/nemovitosti/${task.propertyId}/jednotky/${task.unit.id}`}>{task.unit.label}</Link>}{task.tenant&&<Link href={`/najemnici/${task.tenant.id}`}><UserRound size={13}/>{task.tenant.name}</Link>}{task.lease&&<Link href={`/smlouvy/${task.lease.id}`}><FileText size={13}/>{task.lease.contractNumber||`VS ${task.lease.variableSymbol}`}</Link>}</div></div><span className={`status large-status ${task.status==="DONE"?"ok":task.status==="WAITING"?"warn":"bad"}`}>{statusLabel}</span></div>
    ...
    <div className="detail-grid case-layout">
      ...
      <aside className="col-4 stack-column case-sidebar">
        ...
        {propertyWide&&task.costs.length>0&&<section className="card"><h2>Související náklady</h2>{task.costs.map(cost=><p key={cost.id}><Link href={`/nemovitosti/${task.propertyId}/naklady/${cost.id}`}>{cost.title}</Link></p>)}</section>}
      </aside>
    </div>
  </div></Shell>;
}
