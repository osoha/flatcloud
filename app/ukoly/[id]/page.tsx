...
export default async function TaskDetail({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{ok?:string;error?:string}>}){
  ...
  const task=await prisma.task.findUnique({where:{id},include:{property:true,unit:true,lease:{include:{charges:{where:{active:true},include:{allocations:true,securityDepositOffsets:true,creditApplications:true}}}},tenant:true,assignee:true,createdBy:true,conditionPlanExecution:true,costs:{select:{id:true,title:true}},entries:{include:{author:true},orderBy:{createdAt:"desc"}}}});
  if(!task)notFound();
