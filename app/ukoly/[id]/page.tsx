  ...
  return <Shell user={user} taskPropertyId={task.propertyId||undefined} taskLeaseId={task.leaseId||undefined}><div className="page task-case-page">
    ...
    <div className="detail-grid case-layout">
      <div className="card col-8 case-thread-card">
        <TaskDiscussion composer={canManage && <TaskThreadComposer key={task.status} taskId={task.id} people={discussionPeople} currentUserId={user.id} currentUserName={user.name} currentUserAvatarMimeType={user.avatarMimeType} currentUserUpdatedAt={user.updatedAt} collection={task.category==="COLLECTION"} {...composerMode}/> }>
