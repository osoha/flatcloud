...
export default async function TaskDetail({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{ok?:string;error?:string}>}){
  ...
  return <Shell user={user} taskPropertyId={task.propertyId} taskLeaseId={task.leaseId||undefined}><div className="page task-case-page">
    ...
    <div className="page-title case-title"><div><h1>{task.title}</h1><div className="case-context"><Link href={`/nemovitosti/${task.propertyId}/prehled`}><Home size={13}/>{task.property.name}</Link>{task.unit&&<Link href={`/nemovitosti/${task.propertyId}/jednotky/${task.unit.id}`}>{task.unit.label}</Link>}{task.tenant&&<Link href={`/nemovitosti/${task.propertyId}/najemnici/${task.tenant.id}/upravit`}><UserRound size={13}/>{task.tenant.name}</Link>}{task.lease&&<Link href={`/nemovitosti/${task.propertyId}/smlouvy/${task.lease.id}/upravit`}><FileText size={13}/>{task.lease.contractNumber||`VS ${task.lease.variableSymbol}`}</Link>}</div></div><span className={`status large-status ${task.status==="DONE"?"ok":task.status==="WAITING"?"warn":"bad"}`}>{statusLabel}</span></div>
