...
export default async function NewTaskPage({ searchParams }: { searchParams: Promise<{ propertyId?: string; leaseId?: string; ok?: string; error?: string }> }) {
  ...
  const properties = await prisma.property.findMany({
    ...
    include: {
      ...
      units: { orderBy: { label: "asc" }, include: { leases: { where: { status: { in: ["ACTIVE", "FUTURE"] } }, orderBy: { startDate: "desc" }, include: { tenant: { select: { id: true, name: true } } } } } },
    },
  });
