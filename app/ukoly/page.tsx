...
export default async function TasksPage() {
  ...
  return <Shell user={user}><div className="page">
    <div className="page-title"><div><h1>Úkoly a případy</h1><p>Jedno místo pro provozní úkoly, vymáhání nájemného a komunikaci mezi správcem a vlastníkem.</p></div></div>
    ...
    <div className="card portfolio-table-card"><div className="table-toolbar"><div><h2>Pracovní fronta</h2><p>Vlastník vidí stav případu a průběžné zápisy bez nutnosti obvolávat správce.</p></div></div>
      ...
      {tasks.length ? tasks.map((task)=><tr key={task.id}><td><strong>{task.title}</strong><span className="owner-sub">{task.tenant?.name || `${task._count.entries} záznamů ve vlákně`}</span></td><td><Link className="entity-link" href={`/nemovitosti/${task.propertyId}/prehled`}>{task.property.name}</Link><span className="owner-sub">{task.unit?.label || "Celý objekt"}</span></td><td>{taskCategories[task.category]}</td><td>{task.assignee?.name || "Nepřiřazen"}</td><td>{task.dueAt ? date(task.dueAt) : "—"}</td><td><span className={`status ${task.priority==="URGENT"?"bad":task.priority==="HIGH"?"warn":""}`}>{taskPriorities[task.priority]}</span></td><td><span className={`status ${task.status==="DONE"?"ok":task.status==="WAITING"?"warn":task.status==="CANCELLED"?"":"bad"}`}>{taskStatuses[task.status]}</span></td><td><Link className="table-link" href={`/ukoly/${task.id}`}>Otevřít vlákno</Link></td></tr>) : <tr><td colSpan={8} className="table-empty">Žádné úkoly nejsou evidovány.</td></tr>}
      </tbody></table></div>
    </div>
  </div></Shell>;
}
