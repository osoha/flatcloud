...
export default async function ReportDesignTemplateVersionPage({ params, searchParams }: { params: Promise<{ versionId: string }>; searchParams: Promise<{ ok?: string; error?: string }> }) {
  ...
  return <Shell user={user}><div className="page"><div className="breadcrumb"><Link href="/reporty/sablony">← Šablony</Link></div><div className="page-title"><div><PageHeading>{version.template.name} · v{version.version}</PageHeading><p>{version.template.code} · {version.template.type}</p><span className="status">{version.status}</span></div></div><Flash ok={query.ok} error={query.error}/>
    <div className="card"><div className="form-actions">{editable ? <><form action={action} method="post"><button className="secondary" name="action" value="apply-current-preset">Použít aktuální FlatCloud preset</button></form><form action={action} method="post"><button className="primary" name="action" value="activate">Aktivovat verzi</button></form></> : <form action={action} method="post"><button className="secondary" name="action" value="clone">Klonovat do nové DRAFT</button></form>}</div></div>
