...
export default async function ShareholderReportsPage() {
  ...
  return <Shell user={user}><div className="page shareholder-reports-page">
    <div className="page-title"><div><PageHeading>Akcionářské reporty</PageHeading><p>Interní příprava, kontrola a publikace pravidelných výstupů pro akcionáře.</p></div>{user.role === "SUPER_ADMIN" && <Link className="secondary" href="/reporty/sablony"><LayoutTemplate size={15}/> Šablony</Link>}</div>
    <div className="shareholder-report-grid">
      <Link className="card shareholder-report-card" href="/reporty/kvartalni"><span className="admin-module-icon"><CalendarRange/></span><div><span className="eyebrow">Aktivní workflow</span><h2>Kvartální reporty</h2><p>Reportovací skupiny, datové datové záznamy, redakční kontrola a verzovaná publikace.</p><div className="shareholder-card-meta"><span>{groups.length} {groups.length === 1 ? "skupina" : "skupin"}</span><span>{latest ? `Poslední ${latest.year} Q${latest.quarter}` : "Zatím bez reportu"}</span></div><strong>Otevřít kvartální reporty →</strong></div></Link>
