...
export default async function QuarterlyReportingPage({ searchParams }: { searchParams: Promise<{ ok?: string; error?: string }> }) {
  ...
  return <Shell user={user}><div className="page"><div className="breadcrumb"><Link href="/reporty/akcionarske">Akcionářské reporty</Link><span>›</span><span>Kvartální reporty</span></div><div className="page-title"><div><h1>Kvartální reporty</h1><p>Interní pracovní prostor reportovacích skupin a jejich kvartálních revizí.</p></div>{user.role === "SUPER_ADMIN" && <Link className="button secondary" href="/reporty/sablony">Šablony reportů</Link>}</div><Flash ok={query.ok} error={query.error}/>
