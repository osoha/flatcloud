...
import { listQuarterlyPropertyPhotoCandidates } from "@/lib/reporting/quarterly-report-media-service";
...
import { quarterlyPeriodState } from "@/lib/reporting/quarterly-report-service";
...
export default async function QuarterlyReportWorkspace({ params, searchParams }: { params: Promise<{ groupId: string; reportId: string }>; searchParams: Promise<{ ok?: string; error?: string; section?: string; propertyId?: string }> }) {
  ...
  const report = await prisma.quarterlyReport.findFirst({
    ...
    select: {
      ...
      designTemplateVersion: { select: { id: true, version: true, template: { select: { name: true } } } },
      reportingGroup: { select: { id: true, name: true } },
      propertyReports: { select: {
        id: true, propertyId: true, propertyNameSnapshot: true, propertyAddressSnapshot: true, propertyStatus: true, managementCommentary: true, additionalCommentary: true, technicalSections: true, valuationRows: true,
        media: { where: { OR: [{ role: "PRIMARY", sortOrder: 0 }, { role: "SECONDARY", sortOrder: 0 }] }, select: { id: true, role: true, sortOrder: true, caption: true, sourceDocumentId: true }, orderBy: [{ role: "asc" }, { sortOrder: "asc" }] },
  ...
  if (!report) notFound();
  const periodState = quarterlyPeriodState(report.asOfDate);
  ...
  const activeBaseAction = active ? `/api/reporting-groups/${groupId}/quarterly-reports/${reportId}/properties/${active.row.propertyId}` : "";
  ...
  const photoCandidates = report.status === "DRAFT" && activeSection === "property" && active ? await listQuarterlyPropertyPhotoCandidates({ reportId, reportingGroupId: groupId, propertyId: active.row.propertyId }, user) : [];
  ...
  const activeTemplates = await prisma.reportDesignTemplateVersion.findMany({ where: { status: "ACTIVE", template: { active: true, type: "QUARTERLY_PROPERTY" } }, select: { id: true, version: true, template: { select: { name: true } } }, orderBy: [{ template: { name: "asc" } }, { version: "desc" }] });

  return <Shell user={user}><div className="page quarterly-report-page"><div className="breadcrumb"><Link href={`/reporty/kvartalni/${groupId}`}>← {report.reportingGroup.name}</Link></div><div className="page-title"><div><PageHeading>{report.year} Q{report.quarter}</PageHeading><p>{report.reportingGroup.name} · revize {report.revision} · rozhodné datum {businessDateKey(report.asOfDate)}</p><span className="status">{statusLabels[report.status]}</span></div></div><Flash ok={query.ok} error={query.error}/>{periodState.open&&<div className="notice annual-readiness warn"><strong>Reportovací období ještě není uzavřené</strong><span>Rozhodné datum je {periodState.reportDate}, ale dostupná data jsou zatím pouze do {periodState.dataThrough}. Koncept lze připravovat, odeslání ke kontrole a publikace jsou do uzavření období blokované.</span></div>}
    <div className="quarterly-workspace-layout"><QuarterlyReportWorkspaceNav baseHref={baseHref} activeSection={activeSection} activePropertyId={activeSection === "property" ? active?.row.propertyId : undefined} properties={properties}/><div className="report-workspace-content">
      {activeSection === "overview" && <QuarterlyReportQuarterOverview year={report.year} quarter={report.quarter} groupName={report.reportingGroup.name} revision={report.revision} statusLabel={statusLabels[report.status]} properties={properties} quality={qualityGate} executiveSummaryEditor={executiveSummaryEditor} template={report.designTemplateVersion ? { name: report.designTemplateVersion.template.name, version: report.designTemplateVersion.version } : null} activeTemplates={activeTemplates.map((item) => ({ id: item.id, name: item.template.name, version: item.version }))} editable={report.status === "DRAFT"} templateAction={`/api/reporting-groups/${groupId}/quarterly-reports/${reportId}/template`}/>}
      {activeSection === "property" && active && <QuarterlyReportPropertyWorkspace property={{ propertyId: active.row.propertyId, propertyName: active.row.propertyNameSnapshot, propertyAddress: active.row.propertyAddressSnapshot, propertyStatus: active.row.propertyStatus, managementCommentary: active.row.managementCommentary, additionalCommentary: active.row.additionalCommentary, technicalSections: active.technical, valuationRows: active.valuations, primaryPhoto: active.row.media.find((item) => item.role === "PRIMARY" && item.sortOrder === 0) || null, supportivePhoto: active.row.media.find((item) => item.role === "SECONDARY" && item.sortOrder === 0) || null, snapshot: { id: active.row.snapshot.id, revision: active.row.snapshot.revision, source: active.row.snapshot.source, schemaVersion: active.row.snapshot.schemaVersion, calculatorVersion: active.row.snapshot.calculatorVersion, createdAt: active.row.snapshot.createdAt, sourceNote: active.row.snapshot.sourceNote, quality: active.row.snapshot.quality, data: active.snapshotData } }} candidates={candidates.filter((candidate) => candidate.propertyId === active.row.propertyId)} photoCandidates={photoCandidates} editable={report.status === "DRAFT"} baseAction={activeBaseAction} previewHref={`${baseHref}/nahled/${active.row.propertyId}`} previous={activePropertyIndex > 0 ? { id: parsedProperties[activePropertyIndex - 1].row.propertyId, name: parsedProperties[activePropertyIndex - 1].row.propertyNameSnapshot } : undefined} next={activePropertyIndex < parsedProperties.length - 1 ? { id: parsedProperties[activePropertyIndex + 1].row.propertyId, name: parsedProperties[activePropertyIndex + 1].row.propertyNameSnapshot } : undefined}/>}
