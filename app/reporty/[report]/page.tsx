...
const reportTitles = {
  ...
  saldo: ["Saldo a dlužníci", "Pohledávky po splatnosti podle nájemních vztahů včetně ukončených smluv."],
} as const;
...
export default async function ReportPage({ params, searchParams }: { params: Promise<{ report: string }>; searchParams: Promise<{ properties?: string; propertyId?: string }> }) {
  ...
  const debtRows = charges.filter((row) => overdueDebtCents(row.charge) > 0);
  ...
  const propertyRows = properties.map((property) => {
    ...
    const propertyDebt = charges.filter((row) => row.property.id === property.id).reduce((sum, row) => sum + overdueDebtCents(row.charge), 0);
    return { property, expected: propertyExpected, paid: propertyPaid, debt: propertyDebt, rate: propertyExpected ? Math.round(propertyPaid / propertyExpected * 100) : 100, activeLeases: property.units.flatMap((unit) => unit.leases).filter((lease) => leaseStatusAt(lease) === "ACTIVE").length };
  });

  const debtorMap = new Map<string, { propertyId: string; propertyName: string; unitId: string; unitLabel: string; leaseId: string; tenantName: string; leaseStatus: string; total: number; paid: number; debt: number; oldestDue: Date; count: number }>();
  for (const row of debtRows) {
    const existing = debtorMap.get(row.lease.id);
    const debt = overdueDebtCents(row.charge);
    if (existing) {
    ...
    } else debtorMap.set(row.lease.id, { propertyId: row.property.id, propertyName: row.property.name, unitId: row.unit.id, unitLabel: row.unit.label, leaseId: row.lease.id, tenantName: row.lease.tenant.name, leaseStatus: leaseStatusAt(row.lease), total: row.charge.amountCents, paid: row.paid, debt, oldestDue: row.charge.dueDate, count: 1 });
  }
  ...
  return <Shell user={user}><div className="page"><div className="breadcrumb"><Link href="/portfolio">Portfolio</Link>{propertyScope&&<><span>›</span><Link href={`/nemovitosti/${propertyScope.id}/prehled`}>{propertyScope.name}</Link></>}<span>›</span><span>Reporty</span><span>›</span><span>{baseTitle}</span></div><div className="page-title report-title"><div><h1>{title}</h1><p>{description}</p></div><Link className="secondary" href={backHref}>{backLabel}</Link></div><div className="stat-grid report-stat-grid"><div className="card stat"><div className="stat-icon blue"><Building2/></div><div><span>{propertyScope ? "Jednotky" : "Nemovitosti"}</span><strong>{propertyScope ? propertyScope.units.length : properties.length}</strong><small>{propertyScope ? `${propertyScope.units.flatMap((unit) => unit.leases).filter((lease) => leaseStatusAt(lease) === "ACTIVE").length} aktivních smluv` : "v rozsahu uživatele"}</small></div></div><div className="card stat"><div className="stat-icon orange"><WalletCards/></div><div><span>Předpis {period}</span><strong>{money(expected)}</strong><small>{currentRows.length} předpisů</small></div></div><div className="card stat"><div className="stat-icon green"><TrendingUp/></div><div><span>Uhrazeno</span><strong>{money(paid)}</strong><small className="good">{expected ? Math.round(paid / expected * 100) : 100} % inkaso</small></div></div><div className="card stat"><div className="stat-icon red">!</div><div><span>Dluh po splatnosti</span><strong className={debtTotal ? "negative" : "positive"}>{money(debtTotal)}</strong><small className="bad">{debtors.length} dlužníků</small></div></div></div><div className="card report-chart-card"><div className="card-head"><div><h2>Vývoj za posledních 12 měsíců</h2><p className="muted-copy">Předpis a skutečně alokované úhrady podle období.</p></div></div><CollectionChart data={trend}/></div>
