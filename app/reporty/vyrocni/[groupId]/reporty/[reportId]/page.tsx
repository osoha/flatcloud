...
export default async function AnnualReportWorkspace({ params, searchParams }: { params: Promise<{ groupId: string; reportId: string }>; searchParams: Promise<{ section?: string; propertyId?: string; ok?: string; error?: string }> }) {
  ...
  const [users, globalParentOwner] = await Promise.all([prisma.user.findMany({ where: { active: true }, select: { id: true, name: true, email: true, title: true, role: true, avatarMimeType: true, isTestIdentity: true }, orderBy: { name: "asc" } }), prisma.owner.findFirst({ where: { active: true, affiliation: "FLATCLOUD_PARENT" }, select: { name: true, type: true, affiliation: true, ico: true, address: true, email: true, phone: true } })]);
  const defaultTeam = users.filter((member) => !member.isTestIdentity && ["PROPERTY_MANAGER", "MANAGER", "SUPER_ADMIN"].includes(member.role));
  ...
  return <Shell user={user}><div className="page annual-report-workspace-page">
    ...
    <section className={`annual-q4-alignment ${q4Alignment.status === "ALIGNED" ? "aligned" : "attention"}`} aria-label="Kontrola návaznosti Q4">
      {q4Alignment.status === "ALIGNED" ? <CircleCheck size={19}/> : <TriangleAlert size={19}/>}<div><strong>{q4Alignment.status === "ALIGNED" ? "Q4 a výroční rozsah jsou sladěné" : q4Alignment.status === "NO_PUBLISHED_Q4" ? "Publikovaný Q4 report není k dispozici" : "Q4 a výroční podklad vyžadují kontrolu"}</strong><span>{q4Alignment.status === "ALIGNED" ? `Publikovaný Q4 report, revize ${publishedQ4!.revision}, používá stejné rozhodné datum a stejný rozsah ${report.propertyReports.length} nemovitostí.` : q4Alignment.status === "NO_PUBLISHED_Q4" ? "Výroční report má vlastní zmrazené datové záznamy. Pro redakční srovnání ale zatím chybí publikovaný Q4 report stejné skupiny a roku." : `Shoda data: ${q4Alignment.dateMatches ? "ano" : "ne"} · shoda rozsahu: ${q4Alignment.scopeMatches ? "ano" : "ne"}. Výroční datové záznamy se automaticky nemění.`}</span></div>{publishedQ4&&<Link href={`/reporty/kvartalni/${groupId}/reporty/${publishedQ4.id}`}>Otevřít Q4 report</Link>}
    </section>
    <div className="annual-report-workspace-layout">
      <nav className="annual-report-nav" aria-label="Příprava výročního reportu">
        ...
        <Link className={section === "appendix" ? "active" : ""} href={`${baseHref}?section=appendix`}><strong>05 · Příloha a zdroje</strong><small>Datový záznamy a provenience</small></Link>
      ...
      <div className="report-workspace-content">
        ...
        {section === "property" && selectedProperty && <div className="annual-report-editor-stack">
          ...
          <form className="card annual-report-editor" action={`/api/reporting-groups/${groupId}/annual-reports/${reportId}/properties/${selectedProperty.propertyId}`} method="post">
            <div className="annual-editor-section"><span className="annual-section-number">02</span><div><h3>Hodnota a exit nemovitosti</h3><p className="muted-copy">Roční redakční hodnoty jsou oddělené od provozního datového záznamu a mají vlastní poznámku ke zdroji.</p></div></div>
            <div className="annual-money-grid">
