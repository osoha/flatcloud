...
export default async function LeasesPage({ searchParams }: { searchParams: Promise<{ q?: string; view?: string; propertyId?: string; properties?: string }> }) {
  ...
  return <Shell user={user}><div className="page"><div className="breadcrumb"><Link href="/portfolio">Portfolio</Link><span>›</span><span>Smlouvy</span></div>
    <div data-guide="contracts" className="page-title"><div><PageHeading>Smlouvy</PageHeading><p>{portfolioSelectionLabel(selection, selectedIds.length, properties.length)}.</p></div><div className="action-row"><Link className="secondary" href={alertsHref}>Expirace a výročí</Link><Link className="primary" href={newLeaseHref}>Nová smlouva</Link></div></div>
