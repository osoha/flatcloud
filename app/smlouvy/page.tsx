...
export default async function LeasesPage({ searchParams }: { searchParams: Promise<{ q?: string; view?: string; propertyId?: string; properties?: string }> }) {
  ...
  return <Shell user={user}><div className="page"><div className="breadcrumb"><Link href="/portfolio">Portfolio</Link><span>›</span><span>Smlouvy</span></div>
    <div className="page-title"><div><h1>Smlouvy</h1><p>{portfolioSelectionLabel(selection, selectedIds.length, properties.length)}.</p></div><div className="action-row"><Link className="secondary" href={alertsHref}>Expirace a výročí</Link><Link className="primary" href="/smlouvy/nova">Nová smlouva</Link></div></div>
