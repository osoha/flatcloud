...
export default async function LeasesPage({ searchParams }: { searchParams: Promise<{ q?: string; view?: string; propertyId?: string }> }) {
  ...
  return <Shell user={user}><div className="page"><div className="breadcrumb"><Link href="/portfolio">Portfolio</Link><span>›</span><span>Smlouvy</span></div>
    ...
    <form className="card registry-filter-bar" action="/smlouvy"><input type="hidden" name="view" value={view}/><input name="q" aria-label="Hledat ve smlouvách" defaultValue={query.q || ""} placeholder="Nájemník, e-mail, nemovitost, jednotka, smlouva nebo VS…"/><select name="propertyId" aria-label="Filtrovat podle nemovitosti" defaultValue={propertyId}><option value="">Všechny dostupné nemovitosti</option>{properties.map((property)=><option value={property.id} key={property.id}>{property.name}</option>)}</select><button className="secondary">Filtrovat</button></form>
    <nav className="registry-tabs" aria-label="Stav smluv">{views.map(([key,label])=><Link aria-current={view === key ? "page" : undefined} className={`registry-tab ${view === key ? "active" : ""}`} href={href(key)} key={key}>{label}</Link>)}<Link className="registry-tab" href={`/smlouvy/upozorneni${propertyId ? `?propertyId=${propertyId}` : ""}`}>Expirace a výročí</Link></nav>
