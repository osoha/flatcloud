...
export default async function LeaseAlertsPage({ searchParams }: { searchParams: Promise<{ propertyId?: string; properties?: string; kind?: string }> }) {
  ...
  return <Shell user={user} taskPropertyId={propertyId || undefined}><div className="page">
    ...
    <div className="page-title"><div><PageHeading>Expirace a výročí smluv{scope}</PageHeading><p>Aktivní nájemní smlouvy s koncem nebo nejbližším výročím v následujících 3 kalendářních měsících.</p></div>{propertyId && <Link className="secondary" href={href("/smlouvy/upozorneni", undefined, false)}>Zobrazit celý zvolený rozsah</Link>}</div>
