...
export default async function LeaseAlertsPage({ searchParams }: { searchParams: Promise<{ propertyId?: string }> }) {
  ...
  const alerts = leaseAlertsForProperties(properties);
  const expiry = alerts.filter((row) => row.kind === "EXPIRY").length;
  const anniversaries = alerts.filter((row) => row.kind === "ANNIVERSARY").length;
