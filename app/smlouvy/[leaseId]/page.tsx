...
export default async function LeaseDetail({ params, searchParams }: { params: Promise<{ leaseId: string }>; searchParams: Promise<{ ok?: string; error?: string }> }) {
  ...
  return <Shell user={user} taskPropertyId={lease.unit.propertyId} taskLeaseId={lease.id}><div className="page">
    ...
    <div className="page-title lease-title"><div><h1>{lease.contractNumber || "Smlouva"}</h1><p>{lease.tenant.name} · {lease.unit.property.name} · {lease.unit.label}</p></div><div className="lease-action-bar"><Link className="secondary" href={`/nemovitosti/${lease.unit.propertyId}/predpisy/${lease.id}`}>Předpisy</Link><Link className="secondary" href={`/ukoly/novy?propertyId=${lease.unit.propertyId}&leaseId=${lease.id}`}>Nový úkol</Link>{canEdit&&<Link className="primary" href={`/nemovitosti/${lease.unit.propertyId}/smlouvy/${lease.id}/upravit`}>Upravit smlouvu</Link>}{canEdit&&lifecycleState!=="ENDED"&&<Link className="secondary lifecycle-link" href={`/nemovitosti/${lease.unit.propertyId}/smlouvy/${lease.id}/upravit#lifecycle`}>Ukončit vztah</Link>}</div></div>
    ...
    <div className="detail-grid">
      <div className="card col-6"><h2>Vztah a smlouva</h2><div className="summary-list"><div><span>Nájemník</span><strong><Link href={`/najemnici/${lease.tenantId}`}>{lease.tenant.name}</Link></strong></div><div><span>Nemovitost</span><strong><Link href={`/nemovitosti/${lease.unit.propertyId}/prehled`}>{lease.unit.property.name}</Link></strong></div><div><span>Jednotka</span><strong>{lease.unit.label}</strong></div><div><span>Stav</span><strong>{leaseStatuses[leaseStatusAt(lease)]}</strong></div><div><span>Smlouva platná od</span><strong>{date(lease.startDate)}</strong></div><div><span>Platnost do</span><strong>{effectiveEnd ? date(effectiveEnd) : "neurčito"}</strong></div><div><span>Finanční evidence od</span><strong>{lease.financialTrackingFromPeriod}</strong></div></div>{lease.financialTrackingFromPeriod>lease.startDate.toISOString().slice(0,7)&&<p className="notice">FlatCloud neeviduje předpisy ani úhrady této smlouvy před uvedeným měsícem finanční evidence.</p>}</div>
    ...
    <div className="card settlement-card" id="vyuctovani"><div className="card-head"><div><h2>Vyúčtování / přeplatky</h2><p className="muted-copy">Nedoplatek vytvoří nový předpis. Přeplatek zůstává evidován a lze jej započíst proti otevřeným předpisům.</p></div></div>
