...
export default async function ServiceSettlementPreviewPage({ params, searchParams }: { params: Promise<{ leaseId: string }>; searchParams: Promise<{ from?: string; to?: string }> }) {
  ...
  return <Shell user={user}><div className="page service-settlement-page">
    ...
    <div className="card settlement-next-step"><div><h2>Další krok: vystavení protokolu</h2><p className="muted-copy">Vystavení bude samostatný potvrzovací krok. Teprve ten zmrazí podklady a vytvoří nedoplatek nebo přeplatek; tento pracovní náhled je stále bez zápisu.</p></div><button className="primary" disabled>Vystavit protokol · naváže R5B</button></div>
