...
export default async function ServiceSettlementPreviewPage({ params, searchParams }: { params: Promise<{ leaseId: string }>; searchParams: Promise<{ from?: string; to?: string; ok?: string; error?: string }> }) {
  ...
  return <Shell user={user} taskPropertyId={preview.lease.unit.propertyId} taskLeaseId={preview.lease.id}><div className="page service-settlement-page">
    ...
    {protocols.length>0&&<div className="card settlement-protocol-history"><div className="card-head"><div><h2>Uložené protokoly</h2><p className="muted-copy">Neměnná historie smlouvy. Pracovní verze nejsou schváleným vyúčtováním.</p></div></div><div className="revision-list">{protocols.map((protocol)=><Link href={`/smlouvy/${leaseId}/vyuctovani/${protocol.id}`} key={protocol.id}><span><strong>{date(protocol.periodFrom)} – {date(protocol.periodTo)}</strong><small>Vystavil/a {protocol.issuedBy.name} · {date(protocol.issuedAt)}</small></span><span className={`status ${protocol.balanceCents<=0?"ok":"warn"}`}>{parseServiceSettlementSnapshot(protocol.snapshot).schemaVersion === 2 ? "Pracovní podklad · neúplné" : protocol.balanceCents>0?`Nedoplatek ${money(protocol.balanceCents)}`:protocol.balanceCents<0?`Přeplatek ${money(Math.abs(protocol.balanceCents))}`:"Vyrovnáno"}</span></Link>)}</div></div>}
