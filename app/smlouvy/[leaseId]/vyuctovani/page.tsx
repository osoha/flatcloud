...
import { listServiceSettlementProtocols, parseServiceSettlementSnapshot } from "@/lib/service-settlement-protocols";
import { contractingPartyNames } from "@/lib/lease-parties";
import { editableUnitWhere } from "@/lib/access";
import { sourcePropertyWhere } from "@/lib/settlement-sources";
...
export default async function ServiceSettlementPreviewPage({ params, searchParams }: { params: Promise<{ leaseId: string }>; searchParams: Promise<{ from?: string; to?: string; ok?: string; error?: string }> }) {
  ...
  const canReadSources = await prisma.property.findFirst({ where: { id: preview.lease.unit.propertyId, ...sourcePropertyWhere(user) }, select: { id: true } });
  ...
  return <Shell user={user} taskPropertyId={preview.lease.unit.propertyId} taskLeaseId={preview.lease.id}><div className="page service-settlement-page">
    ...
    <div className="settlement-summary-grid"><Summary label="Předepsané zálohy" value={money(preview.advancesCents)} note={`${preview.advanceRows.length} měsíčních položek`}/><Summary label="Skutečné náklady" value={preview.costRows.length ? money(preview.actualCostsCents) : "Chybí podklady"} note={`${preview.costRows.length} přiřazených podkladů`}/><Summary label={resultLabel} value={preview.costRows.length ? money(Math.abs(preview.balanceCents)) : "Nelze určit"} note="náklady − předepsané zálohy" tone={preview.balanceCents > 0 ? "warn" : preview.balanceCents < 0 ? "ok" : ""}/></div>
    {(preview.blockers.length>0||preview.warnings.length>0)&&<div className="settlement-readiness"><div className="card"><h2>Před vystavením doplnit</h2>{preview.blockers.length?<><p className="muted-copy"><strong>Pracovní podklad není kompletní vyúčtování:</strong></p>{preview.blockers.map((message)=><p className="legal-warning" key={message}>{message}</p>)}</>:<p className="notice"><strong>Žádný blokátor</strong><span>Zdroje jsou připravené k další kontrole.</span></p>}</div><div className="card"><h2>Upozornění ke kontrole</h2>{preview.warnings.length?preview.warnings.map((message)=><p className="notice" key={message}>{message}</p>):<p className="muted-copy">Bez dalších upozornění.</p>}</div></div>}
    <div className="card settlement-source-card"><div className="card-head"><div><h2>Skutečné náklady a způsob rozdělení</h2><p className="muted-copy">{preview.usesConfirmedSources ? "Potvrzené nákladové řádky příslušné smlouvy. Účetní OPEX náklady se k nim nepřičítají." : "Orientační skutečné OPEX náklady kategorie Energie a služby v zadaném období."}</p></div><Link className="secondary" href={`/nemovitosti/${preview.lease.unit.propertyId}/finance#naklady`}>Otevřít náklady objektu</Link></div><div className="table-wrap"><table><thead><tr><th>Datum</th><th>Zdroj</th><th>{preview.usesConfirmedSources ? "Náklad řádku" : "Celý doklad"}</th><th>{preview.usesConfirmedSources ? "Přiřazeno smlouvě" : "Přiřazeno jednotce"}</th><th>Pravidlo</th><th>Doklady</th></tr></thead><tbody>{preview.costRows.length?preview.costRows.map((row)=><tr key={row.id}><td>{businessDateKey(row.effectiveAt)}</td><td><strong>{row.title}</strong></td><td>{money(row.sourceAmountCents)}</td><td>{money(row.allocatedAmountCents)}</td><td>{row.allocationLabel}</td><td>{row.documentCount}</td></tr>):<tr><td className="table-empty" colSpan={6}>Žádné použitelné náklady.</td></tr>}</tbody></table></div></div>
    <SettlementEvidenceTable rows={preview.evidenceRows}/>
    ...
    <div className="card settlement-source-card"><div className="card-head"><div><h2>Odečty měřidel</h2><p className="muted-copy">Kontrolní podklad; náklad se zatím počítá z uloženého rozdělení účetního dokladu.</p></div><Link className="secondary" href={`/nemovitosti/${preview.lease.unit.propertyId}/jednotky/${preview.lease.unitId}`}>Otevřít jednotku</Link></div><div className="table-wrap"><table><thead><tr><th>Měřidlo</th><th>Počáteční stav</th><th>Koncový stav</th><th>Spotřeba</th></tr></thead><tbody>{preview.meterRows.length?preview.meterRows.map((row)=><tr key={row.id}><td><strong>{row.label}</strong></td><td>{row.opening?`${row.opening.value.toLocaleString("cs-CZ")} · ${businessDateKey(row.opening.readAt)} · ${readingMethods[row.opening.method as keyof typeof readingMethods]}`:"Chybí"}</td><td>{row.closing?`${row.closing.value.toLocaleString("cs-CZ")} · ${businessDateKey(row.closing.readAt)} · ${readingMethods[row.closing.method as keyof typeof readingMethods]}`:"Chybí"}</td><td>{row.consumption==null?"—":`${row.consumption.toLocaleString("cs-CZ")} ${row.unitOfMeasure}`}</td></tr>):<tr><td className="table-empty" colSpan={4}>Jednotka nemá evidovaná měřidla.</td></tr>}</tbody></table></div></div>
    <div className="card settlement-next-step"><div><h2>Uložit pracovní protokol</h2><p className="muted-copy">Uložení zachová aktuální podklady včetně chybějících údajů. Jde o interní pracovní verzi, nikoli kompletní vyúčtování pro nájemníka. Nevytváří předpis, kredit ani platbu.</p></div>{existing?<Link className="primary" href={`/smlouvy/${leaseId}/vyuctovani/${existing.id}`}>Otevřít protokol překrývajícího se období</Link>:canIssue&&!periodError?<form className="settlement-issue-form" method="post" action={`/api/properties/${preview.lease.unit.propertyId}/leases/${leaseId}/service-settlements`}><input type="hidden" name="from" value={preview.period.from}/><input type="hidden" name="to" value={preview.period.to}/><label className="checkbox-field"><input type="checkbox" name="confirm"/><span>Zkontroloval/a jsem zdroje a beru na vědomí, že ukládám neúplný pracovní podklad.</span></label><button className="primary">Uložit bez zaúčtování</button></form>:<button className="primary" disabled>{canIssue?"Opravte zadané období":"Pouze ke čtení"}</button>}</div>
    {protocols.length>0&&<div className="card settlement-protocol-history"><div className="card-head"><div><h2>Uložené protokoly</h2><p className="muted-copy">Neměnná historie smlouvy. Pracovní verze nejsou schváleným vyúčtováním.</p></div></div><div className="revision-list">{protocols.map((protocol)=><Link href={`/smlouvy/${leaseId}/vyuctovani/${protocol.id}`} key={protocol.id}><span><strong>{businessDateKey(protocol.periodFrom)} – {businessDateKey(protocol.periodTo)}</strong><small>Vystavil/a {protocol.issuedBy.name} · {date(protocol.issuedAt)}</small></span><span className={`status ${protocol.balanceCents<=0?"ok":"warn"}`}>{parseServiceSettlementSnapshot(protocol.snapshot).schemaVersion === 2 ? "Pracovní podklad · neúplné" : protocol.balanceCents>0?`Nedoplatek ${money(protocol.balanceCents)}`:protocol.balanceCents<0?`Přeplatek ${money(Math.abs(protocol.balanceCents))}`:"Vyrovnáno"}</span></Link>)}</div></div>}
