...
import { canonicalizeAccessScope, isPropertyLocalInvitation, rotateInvitation } from "@/lib/user-access-management";
...
export async function POST(request: Request, { params }: { params: Promise<{ inviteId: string }> }) {
  ...
  if (!global && (!(await requirePropertyAdmin(old.propertyId)) || !isPropertyLocalInvitation(old, old.propertyId))) return go(request, "/portfolio");
  ...
  try {
    ...
    const scope = canonicalizeAccessScope({ role, permission, propertyIds, unitIds, allProperties });
    const { invitation, token } = await rotateInvitation({ replaceId: old.id, email: old.email, name: old.name, propertyId: old.propertyId, ...scope, invitedById: actor.id });
    const inviteUrl = redirectUrl(`/pozvanka/${token}`, request).toString();
    const result = await sendInvitationEmail({ to: old.email, inviterName: actor.name, propertyName: scope.allProperties ? "všem nemovitostem FlatCloud" : old.property.name, permissionLabel: propertyPermissions[scope.permission], inviteUrl });
    await audit(actor.id, edit ? "USER_INVITATION_EDITED" : "USER_INVITATION_RESENT", "UserInvitation", invitation.id, { replacedInvitationId: old.id, ...scope, sent: result.sent }, old.propertyId);
