...
import { createDocumentFromUpload, type DocumentContext } from "@/lib/documents/service";
...
export async function POST(request: Request) {
  ...
  const returnTo=value(form,"returnTo")||"/dokumenty";
  try {
    ...
    for(const file of files)await createDocumentFromUpload({...context,actor:user,...file,category:documentCategory(form.get("category"),file),photoStage:documentPhotoStage(form.get("photoStage")),title:value(form,"title")||(files.length===1?file.originalName:file.originalName),description:value(form,"description"),documentDate:value(form,"documentDate")?new Date(`${value(form,"documentDate")}T12:00:00`):undefined});
