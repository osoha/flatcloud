...
export async function POST(request: Request, { params }: { params: Promise<{ id: string; tenantId: string }> }) {
  ...
  try {
    const allowed = await prisma.tenant.findFirst({ where: { id: tenantId, OR: [{ leases: { some: { unit: { propertyId: id } } } }, { leaseParties: { some: { lease: { unit: { propertyId: id } } } } }] }, include: { leases: { where: { unit: { propertyId: id } }, orderBy: { startDate: "desc" }, take: 1 }, leaseParties: { where: { role: "CONTRACTING_PARTY", lease: { unit: { propertyId: id } } }, include: { lease: { select: { unitId: true } } }, orderBy: { createdAt: "desc" }, take: 1 } } });
