...
export async function POST(request: Request, { params }: { params: Promise<{ id: string; leaseId: string }> }) {
  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const application = await tx.leaseCreditApplication.create({ data: { creditId, chargeId, amountCents, createdById: user.id } }); return { application, remainingCents: creditRemaining - amountCents };
    });
