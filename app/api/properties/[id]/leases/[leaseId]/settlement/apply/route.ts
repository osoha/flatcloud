...
export async function POST(request: Request, { params }: { params: Promise<{ id: string; leaseId: string }> }) {
  ...
  try {
    ...
    const result = await serializableTransaction(async (tx) => {
      ...
      if (!credit || !charge) throw new Error("Přeplatek nebo předpis nepatří ke stejné smlouvě."); const creditRemaining = remainingCreditCents(credit); const chargeOutstanding = outstandingCents(charge); if (amountCents <= 0 || amountCents > creditRemaining) throw new Error("Částka převyšuje zbývající přeplatek."); if (amountCents > chargeOutstanding) throw new Error("Částka převyšuje aktuální dluh předpisu.");
  ...
  } catch (error) { return goWithMessage(request, `/smlouvy/${leaseId}`, "error", error instanceof Error ? error.message : "Přeplatek se nepodařilo započíst."); }
}
