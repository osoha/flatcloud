...
export async function POST(request: Request, { params }: { params: Promise<{ id: string; unitId: string; ownershipId: string }> }) {
  ...
  try {
    ...
    await prisma.unitOwnership.update({ where: { id: ownershipId }, data: { shareBasisPoints: Math.round(share * 100), note: text(form, "note") } });
    await audit(access.user.id, "UNIT_OWNER_UPDATED", "UnitOwnership", ownershipId, { propertyId: id, unitId, sharePercent: share });
    return goWithMessage(request, `/nemovitosti/${id}/jednotky/${unitId}/upravit`, "ok", "Podíl vlastníka jednotky byl upraven.");
  } catch (error) {
