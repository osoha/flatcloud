...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const edited = await prisma.user.findUnique({ where: { id }, select: { active: true, role: true } });
    if (!edited) throw new Error("Uživatel nebyl nalezen.");
    if (edited.active && edited.role === UserRole.SUPER_ADMIN && (!active || role !== UserRole.SUPER_ADMIN)) {
      const activeSuperAdmins = await prisma.user.count({ where: { active: true, role: UserRole.SUPER_ADMIN } });
      if (activeSuperAdmins <= 1) {
        throw new Error("Posledního aktivního hlavního administrátora nelze deaktivovat ani změnit jeho roli.");
      }
    }
    ...
    await prisma.$transaction(async (tx) => {
      const current = await tx.user.findUnique({ where: { id }, select: { active: true, role: true } });
    ...
    await audit(admin.id, "USER_UPDATED", "User", id, { email, role, active, allProperties, memberships, avatarChanged: Object.keys(avatarUpdate).length > 0 });
