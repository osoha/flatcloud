...
export async function POST(request:Request,{params}:{params:Promise<{id:string}>}){
  ...
  try{
    ...
    await prisma.$transaction([prisma.task.update({where:{id},data:{status:"DONE"}}),prisma.auditLog.create({data:{userId:user.id,propertyId:task.propertyId,action:"TASK_CLOSED",entityType:"Task",entityId:id,details:{entryId:entry.id,attachmentCount:files.length}}})]);
