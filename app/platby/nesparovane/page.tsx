...
export default async function UnmatchedPaymentsPage() {
  ...
  return <Shell user={user}><div className="page">
    ...
    <div className="page-title"><div><h1>Globální fronta nespárovaných plateb</h1><p>Centrální pracovní fronta pouze pro hlavního administrátora. Obsahuje bankovní transakce čekající na potvrzení i RB e-maily, u kterých nebylo možné určit objekt.</p></div></div>
    ...
    <div className="card portfolio-table-card" style={{marginTop:16}}><div className="table-toolbar"><div><h2>RB e-maily bez objektu</h2><p>Parser rozpoznal e-mail, ale chybí jednoznačné propojení na účet / smlouvu nebo formát notifikace nebyl úplný.</p></div></div><div className="table-wrap"><table><thead><tr><th>Přijato</th><th>Částka</th><th>Cílový účet</th><th>Plátce</th><th>VS</th><th>Výsledek parseru</th><th></th></tr></thead><tbody>{inbox.length ? inbox.map((row) => <tr key={row.id}><td>{date(row.receivedAt)}</td><td className="money">{row.amountCents ? money(row.amountCents) : "—"}</td><td>{row.recipientAccount || "—"}</td><td>{row.counterpartyName || "—"}<span className="owner-sub">{row.counterpartyAccount}</span></td><td>{row.variableSymbol || "—"}</td><td>{row.parseNote || "—"}</td><td><Link className="table-link" href={`/platby/nesparovane/email/${row.id}`}>Vyřešit</Link></td></tr>) : <tr><td colSpan={7} className="table-empty">Žádné e-mailové notifikace nečekají na ruční zásah.</td></tr>}</tbody></table></div></div>
  </div></Shell>;
}
