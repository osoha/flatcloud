...
export default async function UnmatchedPaymentsPage() {
  ...
  const [transactions, inbox, ignored] = await Promise.all([
    ...
    prisma.inboxPayment.findMany({ where: { status: "IGNORED" }, include: { property: true }, orderBy: { receivedAt: "desc" }, take: 100 }),
  ]);
