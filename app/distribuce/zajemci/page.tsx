...
export default async function DistributionCrmPage({
  ...
  const [prospects, properties] = await Promise.all([
    prisma.distributionProspect.findMany({
      where: { active: true },
      include: {
        opportunities: {
          include: {
              ...
              include: {
                property: {
                  select: {
                    id: true,
                    name: true,
                    flatcloudConsolidationBasisPoints: true,
                  },
                },
                valuationSnapshots: {
  ...
  const opportunities = prospects
      ...
      .filter(
        (item) =>
          (item.unit.property.flatcloudConsolidationBasisPoints || 0) > 0,
      ),
    ...
    valuationPrices = Object.fromEntries(
      units.flatMap((unit) =>
        unit.valuationSnapshots[0]
          ? [
              [
                unit.id,
  ...
  return (
    <Shell user={user}>
      <div className="page distribution-crm-page">
        ...
        <div className="page-title">
          ...
          <div className="action-row">
            ...
            <Link className="secondary" href="/distribuce">
              Zpět na jednotky
            </Link>
          </div>
        </div>
        ...
        <div className="stat-grid v21-stat-grid distribution-crm-kpis">
          ...
          />
          <Stat
            label="Nabídky / rezervace"
            value={String(
              open.filter(
                (item) => item.stage === "OFFER" || item.stage === "RESERVED",
              ).length,
            )}
            icon={<UserRound />}
          />
          <Stat
            label="Další krok po termínu"
        ...
        <div className="distribution-crm-create">
          <details className="card create-panel">
            ...
            <form
              action="/api/distribution/prospects"
              method="post"
              className="form-grid"
            >
              ...
              <label className="field">
                <span>Zdroj kontaktu</span>
                <input name="source" placeholder="Doporučení, web, makléř…" />
              </label>
              <label className="field field-full">
                <span>Poznámka</span>
                <textarea name="note" rows={2} />
              </label>
              <button className="primary field-full" type="submit">
                Přidat zájemce
              </button>
            </form>
          </details>
          <details className="card create-panel">
            ...
            <form
              action="/api/distribution/opportunities"
              method="post"
              className="form-grid"
            >
              <label className="field">
              ...
              <label className="field">
                ...
                <select name="stage" defaultValue="NEW">
                  {Object.entries(distributionOpportunityStages).map(
                    ([value, label]) => (
              ...
              <ValuationPricePrefill prices={valuationPrices} />
              <label className="field">
                <span>Nabídka zájemce Kč</span>
                <input
                  name="offeredPrice"
                  type="number"
                  min="0.01"
              ...
              <label className="field">
                <span>Další krok k datu</span>
                <input name="nextActionAt" type="date" />
              </label>
              <label className="field">
                <span>Stav opce</span>
                <select name="optionStatus" defaultValue="NONE">
                  {Object.entries(distributionOptionStatuses).map(
                    ([value, label]) => (
                      <option value={value} key={value}>
                        {label}
                      </option>
              ...
              <label className="field">
                <span>Platnost opce do</span>
                <input name="optionExpiresAt" type="date" />
              </label>
              <label className="field">
                <span>Reference dokumentu</span>
                <input name="optionReference" maxLength={200} />
              </label>
              <label className="field field-full">
                <span>Poznámka</span>
                <textarea name="note" rows={2} />
              ...
              <button
                ...
                type="submit"
                disabled={!prospects.length || !units.length}
              >
                Založit příležitost
              </button>
            </form>
            ...
            <table>
              <thead>
                <tr>
                  ...
                  <th>Jednotka</th>
                  <th>Fáze / opce</th>
                  <th>Nabídková cena</th>
                  <th>Nabídka zájemce</th>
                  <th>Další krok</th>
                  <th>Kontakt</th>
                  <th></th>
                </tr>
                      ...
                      <tr key={item.id} className={isOverdue ? "row-debt" : ""}>
                        <td>
                          ...
                          <span className="owner-sub">
                            {item.prospect.source || "Zdroj neuveden"}
                          </span>
                        </td>
                        <td>
                          <Link
                            className="entity-link"
                            href={`/nemovitosti/${item.unit.property.id}/jednotky/${item.unitId}`}
                          >
                            {item.unit.property.name} · {item.unit.label}
                          </Link>
                        </td>
                        <td>
                          <span
                            className={`status ${item.stage === "WON" ? "ok" : item.stage === "LOST" ? "" : item.stage === "RESERVED" ? "warn" : ""}`}
                          >
                            {distributionOpportunityStages[item.stage]}
                          </span>
                          <span className="owner-sub">
                            {distributionOptionStatuses[item.optionStatus]}
                            {item.optionExpiresAt
                              ? ` · do ${date(item.optionExpiresAt)}`
                              : ""}
                          </span>
                        </td>
                        <td>
                          {item.askingPriceCents
                            ? money(Number(item.askingPriceCents))
                            : "—"}
                        </td>
                        <td>
                          {item.offeredPriceCents
                            ? money(Number(item.offeredPriceCents))
                            : "—"}
                        </td>
                        <td>
                          {item.nextActionAt ? (
                            <strong className={isOverdue ? "negative" : ""}>
                        ...
                        <td>
                          ...
                          >
                            <form
                              action={`/api/distribution/opportunities/${item.id}`}
                              method="post"
                            >
                              <label className="field">
                                <span>Fáze *</span>
                              ...
                              <label className="field">
                                ...
                                <input
                                  name="nextActionAt"
                                  type="date"
                                  defaultValue={dateInput(item.nextActionAt)}
                                />
                              </label>
                              <label className="field">
                                <span>Stav opce</span>
                                <select
                                  name="optionStatus"
                                  defaultValue={item.optionStatus}
                                >
                                  {Object.entries(
                                    distributionOptionStatuses,
                                  ).map(([value, label]) => (
                              ...
                              <label className="field">
                                ...
                                <input
                                  ...
                                  type="date"
                                  defaultValue={dateInput(item.optionExpiresAt)}
                                />
                              </label>
                              <label className="field">
                                <span>Reference dokumentu</span>
                                <input
                                  ...
                                  maxLength={200}
                                  defaultValue={item.optionReference || ""}
                                />
                              </label>
                              <label className="field">
                                <span>Poznámka</span>
                        ...
                        Uložit fázi a další krok včetně opce
                              </button>
                            </form>
                            <div className="distribution-funnel-history">
                              <strong>Historie funnelu</strong>
                              {item.events.map((event) => (
                                <span key={event.id}>
                                  {date(event.createdAt)} ·{" "}
                                  {event.fromStage
                                    ? `${distributionOpportunityStages[event.fromStage]} → `
                                    : "Založeno: "}
                                  {distributionOpportunityStages[event.toStage]}{" "}
                                  ·{" "}
                                  {
                                    distributionOptionStatuses[
                                      event.optionStatus
