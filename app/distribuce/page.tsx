import { unitConditionRatings } from "@/lib/portfolio/unit-condition-assessments";
...
export default async function DistributionPage({ searchParams }: { searchParams: Promise<{ ok?: string; error?: string; properties?: string }> }) {
  ...
  return <Shell user={user}><div className="page distribution-page"><div className="page-title"><div><PageHeading>Interní distribuce</PageHeading><p>Valuace, distribuční připravenost a obchodní pipeline potvrzených aktiv FlatCloud.</p></div><div className="action-row"><Link className="secondary" href="/portfolio/kvalita">Kvalita a CAPEX</Link><Link className="secondary" href="/distribuce/reporting">Distribuční reporting</Link><Link className="secondary" href="/distribuce/uvitaci-dopisy">Uvítací dopisy</Link><Link className="primary" href="/distribuce/zajemci">Přehled zájemců</Link></div></div><Flash ok={query.ok} error={query.error}/>

    <div className="stat-grid v21-stat-grid distribution-kpis"><Stat label="Vybrané jednotky" value={String(units.length)} icon={<Home/>}/><Stat label="Technický podklad" value={`${conditions.length}/${units.length}`} icon={<ClipboardCheck/>}/><Stat label="Valuováno" value={`${valuations.length}/${units.length}`} icon={<Building2/>}/><Stat label="Tržní hodnota jednotek" value={money(marketValue)} icon={<Handshake/>}/><Stat label="Připraveno / otevřené příležitosti" value={`${ready} / ${openOpportunities}`} icon={<Users/>}/></div>
