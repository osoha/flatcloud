...
export default async function SearchPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  const [tasks, transactions] = q && propertyIds.length ? await Promise.all([
  ...
  const count = propertyResults.length + unitResults.length + tenantResults.length + tasks.length + transactions.length;
  ...
  return <Shell user={user}><div className="page">
    ...
    <div className="page-title"><div><h1>Hledání</h1><p>Nemovitosti, jednotky, nájemníci, platby a úkoly v rozsahu vašich oprávnění.</p></div></div>
    <form className="card search-page-form" action="/hledat" method="get">
      <Search size={18}/><input name="q" defaultValue={q} autoFocus placeholder="Hledat nemovitost, nájemníka, VS, platbu nebo úkol…"/><button className="primary" type="submit">Hledat</button>
    </form>
    {!q ? <div className="card empty-state"><h2>Začněte zadáním hledaného výrazu</h2><p>Můžete hledat například název domu, číslo jednotky, jméno nájemníka, variabilní symbol nebo název úkolu.</p></div> : <>
      ...
      <div className="search-grid">
        ...
        <SearchCard title="Nájemníci" empty="Žádný nájemník neodpovídá hledání.">{tenantResults.map((row) => <Link className="search-hit" href={`/nemovitosti/${row.propertyId}/najemnici/${row.tenant.id}/upravit`} key={row.tenant.id}><strong>{row.tenant.name}</strong><span>{row.propertyName} · {row.unitLabel}</span></Link>)}</SearchCard>
