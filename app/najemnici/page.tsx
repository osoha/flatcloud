...
export default async function TenantsPage({ searchParams }: { searchParams: Promise<{ status?: string }> }) {
  ...
  const tenants = await prisma.tenant.findMany({ where: tenantAccessWhere(user), include: { leases: { where: leaseAccessWhere(user), include: { unit: { include: { property: true } } } }, leaseParties: { where: { role: "CONTRACTING_PARTY", lease: leaseAccessWhere(user) }, include: { lease: { include: { unit: { include: { property: true } } } } } } }, orderBy: { name: "asc" } });
  const rows = tenants.map((tenant) => { const relationships = Array.from(new Map([...tenant.leases, ...tenant.leaseParties.map((party) => party.lease)].map((lease) => [lease.id, lease])).values()); const status = relationships.some((lease) => leaseStatusAt(lease) === "ACTIVE") ? "ACTIVE" : relationships.some((lease) => leaseStatusAt(lease) === "FUTURE") ? "FUTURE" : "ENDED"; const relationship = relationships.find((lease) => leaseStatusAt(lease) === "ACTIVE") || relationships.find((lease) => leaseStatusAt(lease) === "FUTURE") || relationships[0]; return { tenant, relationships, status, relationship }; });
