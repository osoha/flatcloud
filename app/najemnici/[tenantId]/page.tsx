...
export default async function TenantDetail({ params }: { params: Promise<{ tenantId: string }> }) {
  ...
  const ledgerRows = await loadPaymentLedgerRows(tenant.leases.map((lease) => lease.id));
  const charges = tenant.leases.flatMap((lease) => lease.charges);
  ...
  const heldDepositCents = tenant.leases.reduce((sum, lease) => sum + securityDepositSnapshot(lease).heldPrincipalCents, 0);
