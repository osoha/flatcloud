...
export default async function TenantDetail({ params }: { params: Promise<{ tenantId: string }> }) {
  ...
  return <Shell user={user}><div className="page">
    ...
    <div className="page-title"><div><PageHeading>{tenant.name}</PageHeading><p>{tenant.type === "COMPANY" ? "Právnická osoba" : "Fyzická osoba"} · {status === "PROFILE" ? "profil bez smlouvy" : leaseStatuses[status]}</p></div><div className="top-actions">{actionProperty && <><Link className="secondary" href={`/nemovitosti/${actionProperty.id}/najemnici/${tenant.id}/upravit`}>Upravit profil</Link><Link className="primary" href={`/nemovitosti/${actionProperty.id}/smlouvy/nova?tenantId=${tenant.id}`}>Přidat do smlouvy</Link></>}</div></div>
