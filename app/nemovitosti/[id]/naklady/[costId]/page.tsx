import { settledCents, expenseKinds, type ExpenseKind } from "@/lib/bank-expense-values";
...
export default async function PropertyCostDetail({ params, searchParams }: { params: Promise<{ id: string; costId: string }>; searchParams: Promise<{ ok?: string; error?: string }> }) {
  ...
  const cost = await prisma.propertyCost.findFirst({
    ...
    include: {
      bankSettlements: { include: { transaction: true }, orderBy: { createdAt: "asc" } },
      unit: true,
      task: true,
      budgetLine: true,
  ...
  const paidCents = settledCents(cost.bankSettlements);
  ...
  return <Shell user={user} taskPropertyId={id}><div className="page">
    ...
    <section className="card" aria-label="Úhrady nákladu"><div className="card-head"><h2>Úhrady nákladu</h2><Link className="secondary" href={`/nemovitosti/${id}/bankovni-vydaje`}>Otevřít bankovní výdaje</Link></div><p>Vznik nákladu: {date(cost.effectiveAt)}. Stav {propertyCostStatuses[cost.status]} sám nepotvrzuje zaplacení.</p><div className="bank-settlement-summary"><div><span>Uhrazeno po vratkách</span><strong>{moneyExact(paidCents)}</strong></div><div><span>{paidCents>cost.amountCents?"Přeplatek k vrácení":"Zbývá uhradit"}</span><strong>{moneyExact(Math.abs(cost.amountCents-paidCents))}</strong></div></div><p>Dobropis: v úpravě nákladu uveďte novou výslednou částku a důvod s číslem dobropisu, přiložte doklad. Bankovní vratku přiřaďte samostatně; sama nepřepisuje náklad.</p>
    {cost.bankSettlements.length>0?<div className="table-wrap"><table><thead><tr><th>Datum úhrady</th><th>Pohyb</th><th>Částka</th><th>Důvod a historie</th></tr></thead><tbody>{cost.bankSettlements.map(row=><tr key={row.id}><td>{date(row.transaction.bookedAt)}</td><td>{expenseKinds[row.kind as ExpenseKind]}</td><td>{moneyExact(row.amountCents)}</td><td>{row.reason}{row.voidedAt&&<p>Stornováno: {row.voidReason}</p>}</td></tr>)}</tbody></table></div>:<p>Zatím bez doložené bankovní úhrady.</p>}
    </section>
    ...
    {canManage&&!cost.conditionPlanExecution&&<details className="card"><summary>Upravit náklad a stav</summary><p>Změna upraví tento náklad. Nevytvoří další závazek ani bankovní platbu. Podíly jednotek a doklady zůstanou zachované.</p><form action={`/api/properties/${id}/costs/${cost.id}`} method="post" className="form-grid" data-testid="cost-edit">
      ...
      <label className="field"><span>Částka v Kč *</span><input name="amount" type="number" min="0" step="0.01" max="21474836.47" defaultValue={(cost.amountCents/100).toFixed(2)} required/></label>
