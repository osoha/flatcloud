...
export default async function EditTenant({ params, searchParams }: { params: Promise<{ id: string; tenantId: string }>; searchParams: Promise<{ ok?: string; error?: string }> }) {
  ...
  return <Shell user={user}><FormPage title={`Upravit nÃ¡jemnÃ­ka: ${tenant.name}`} backHref={backHref}>
    ...
    <FormCard action={`/api/properties/${id}/tenants/${tenant.id}`} cancelHref={`/nemovitosti/${id}/najemnici`}>
      ...
      <Textarea label="ZnÃ¡mÃ© ÃºÄty plÃ¡tce" name="payerAccounts" defaultValue={tenant.payerAccounts.join("\n")} placeholder="Jeden ÃºÄet na Å™Ã¡dek"/>
      <div className="field field-full notice"><strong>NÃ¡jemnÃ­k se neukonÄuje ani nemaÅ¾e</strong><span>Historie nÃ¡jemnÃ­ch vztahÅ¯ zÅ¯stÃ¡vÃ¡ zachovÃ¡na. UkonÄenÃ­ se provÃ¡dÃ­ vÅ¾dy na konkrÃ©tnÃ­ smlouvÄ›.</span></div>
    </FormCard>
  </FormPage></Shell>;
}
