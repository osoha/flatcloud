...
export default async function NewLease({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ ok?: string; error?: string; unitId?: string; tenantId?: string }> }) {
  ...
  return <Shell user={user} taskPropertyId={id}><FormPage title="PÅ™idat nÃ¡jemnÃ­ smlouvu" description="Zvolte jednotku a dobu trvÃ¡nÃ­. FlatCloud pÅ™i uloÅ¾enÃ­ automaticky urÄÃ­ stav smlouvy a zablokuje jakÃ½koli pÅ™ekryv s existujÃ­cÃ­m nÃ¡jemnÃ­m obdobÃ­m." backHref={`/nemovitosti/${id}/smlouvy`}>
