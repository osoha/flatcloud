...
export default async function EditLease({ params, searchParams }: { params: Promise<{ id: string; leaseId: string }>; searchParams: Promise<{ ok?: string; error?: string }> }) {
  ...
  const [property, lease, tenants, query, usedRows] = await Promise.all([
    ...
    prisma.lease.findFirst({ where: { id: leaseId, unit: unitAccessWhere(user, id) }, include: { tenant: true, unit: true, ownerBankAccount: true, parties: { include: { tenant: true }, orderBy: [{ isPrimary: "desc" }, { createdAt: "asc" }] }, paymentItems: true, securityDepositTerms: { orderBy: [{ effectiveFrom: "asc" }, { createdAt: "asc" }] }, charges: { include: { items: true } }, rentChangeProposals: { where: { status: "CONFIRMED", effectiveFrom: { gt: new Date() } }, orderBy: { effectiveFrom: "asc" }, take: 1 } } }),
  ...
  return <Shell user={user} taskPropertyId={id} taskLeaseId={lease.id}><FormPage title={`Upravit smlouvu: ${lease.unit.label}`} description={lease.tenant.name} backHref={`/nemovitosti/${id}/jednotky/${lease.unitId}`}>
    ...
    <div className="card ownership-simple-card" id="lifecycle">
      ...
      </div> : <div className="lifecycle-entry"><div><strong>{lifecycleStatus === "FUTURE" ? "Smlouva ještě nezačala" : "Ukončení vyžaduje kontrolu dopadů"}</strong><p className="muted-copy">Samostatný krok před potvrzením ukáže otevřené předpisy, stav kauce a dopad na budoucí změny nájmu.</p></div><a className="secondary lifecycle-link" href={`/smlouvy/${lease.id}/ukoncit`}>{lifecycleStatus === "FUTURE" ? "Zrušit budoucí smlouvu" : "Pokračovat k ukončení"}</a></div>}
    </div>
  </FormPage></Shell>;
}
