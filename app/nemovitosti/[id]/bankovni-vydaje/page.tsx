...
export default async function BankExpenses({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{year?:string;ok?:string;error?:string;state?:string}>}) {
  ...
  const visible=query.state==="open"?rows.filter(r=>bankRemainder(r.amountCents,r.expenseAllocations)>0):rows;
  return <Shell user={user} taskPropertyId={id}><div className="page">
    ...
    <p className="notice">Přehled obsahuje importované pohyby účtů tohoto domu. Úplnost ověřte proti bankovnímu výpisu. ACTUAL potvrzuje vznik nákladu; zaplacení dokládají až úhrady. NOI a výhled cashflow nadále vycházejí z evidence nákladů.</p>
    <form method="get" className="form-actions"><label>Rok úhrady <input aria-label="Rok úhrady" name="year" type="number" min="2000" max="2100" defaultValue={year}/></label><label>Zobrazení <select name="state" defaultValue={query.state||"all"}><option value="all">Všechny pohyby</option><option value="open">Jen nerozdělené</option></select></label><button className="secondary">Zobrazit</button></form>
    ...
    <section className="card"><h2>Pohyby ({visible.length})</h2>{!visible.length&&<p>Pro vybrané období nejsou žádné pohyby.</p>}{visible.map(r=>{const left=bankRemainder(r.amountCents,r.expenseAllocations);return <details key={r.id} id={`pohyb-${r.id}`} className="card"><summary>{date(r.bookedAt)} · {r.counterpartyName||r.message||r.externalId} · {moneyExact(r.amountCents)} · {left?`zbývá ${moneyExact(left)}`:"Rozděleno"}</summary><p>{r.bankAccount.ibanMasked} · ID {r.externalId} · VS {r.variableSymbol||"—"} · {r.message} · zdroj {r.source}</p>
