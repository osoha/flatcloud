...
type Input = {propertyId:string;unitId:string;meterId:string;readAt:string;value:number;method:string;leaseId?:string|null;note?:string|null;correctsId?:string|null;correctionReason?:string|null;evidenceDocumentId?:string|null};
export async function recordMeterReading(actor:Actor,input:Input) {
  ...
  try {
    return await serializableTransaction(async tx => {
      const meter=await tx.meter.findFirst({where:{id:input.meterId,unitId:input.unitId,unit:editableUnitWhere(actor,input.propertyId)},include:{readings:true}});
      ...
      const leaseId=original ? original.leaseId : input.leaseId || null;
      ...
      const evidence=evidenceId ? await tx.document.findFirst({where:{AND:[documentAccessWhere(actor),{id:evidenceId,propertyId:input.propertyId,OR:[{unitId:input.unitId},{lease:{unitId:input.unitId}}]}]},include:{fileAsset:true}}) : null;
      if(evidenceId && (!evidence || !(evidence.fileAsset.mimeType.startsWith('image/') || evidence.fileAsset.mimeType==='application/pdf'))) throw new Error('Důkaz musí být dostupná fotografie nebo PDF této jednotky.');
