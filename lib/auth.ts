...
export async function actualUser(){const store=await cookies();const token=store.get("fc_session")?.value;if(!token)return null;try{const{payload}=await jwtVerify(token,getSessionSecret());if(typeof payload.userId!=="string")return null;const user=await prisma.user.findFirst({where:{id:payload.userId,active:true},select:{id:true,email:true,name:true,passwordHash:true,sessionVersion:true,role:true,active:true,allProperties:true,flatcloudMember:true,phone:true,title:true,avatarMimeType:true,avatarChoice:true,createdAt:true,updatedAt:true}});return user && sessionVersionMatches(payload.sessionVersion,user.sessionVersion) ? user : null}catch{return null}}
...
export async function previewContext() {
  ...
  try {
    ...
    const target = await prisma.user.findFirst({where:{id:payload.targetId,active:true},select:{id:true,email:true,name:true,passwordHash:true,sessionVersion:true,role:true,active:true,allProperties:true,flatcloudMember:true,phone:true,title:true,avatarMimeType:true,avatarChoice:true,createdAt:true,updatedAt:true}});
