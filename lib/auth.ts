...
export async function currentUser(){const store=await cookies();const token=store.get("fc_session")?.value;if(!token)return null;try{const{payload}=await jwtVerify(token,getSessionSecret());if(typeof payload.userId!=="string")return null;return prisma.user.findFirst({where:{id:payload.userId,active:true},select:{id:true,email:true,name:true,passwordHash:true,role:true,active:true,allProperties:true,phone:true,title:true,avatarMimeType:true,createdAt:true,updatedAt:true}})}catch{return null}}
