...
export const A3A_REPORT_ARTIFACT_MIME_TYPE = "text/plain; charset=utf-8";
export const A3A_REPORT_ARTIFACT_SUFFIX = "a3a-report-artifact.txt";
...
/** Temporary deterministic A3a artifact. A3b replaces only this generator. */
export function generateA3aReportArtifact(report: ArtifactReport) {
  return new TextEncoder().encode([
    "FlatCloud published report artifact — temporary A3a format",
    `reportId=${report.id}`,
    `reportingGroupId=${report.reportingGroupId}`,
...
function storageKey(report: ArtifactReport) {
  return `reporting-groups/${report.reportingGroupId}/quarterly-reports/${report.id}/revision-${report.revision}/${randomUUID()}-${A3A_REPORT_ARTIFACT_SUFFIX}`;
}

function filename(report: ArtifactReport) { return `flatcloud-${report.year}-q${report.quarter}-revize-${report.revision}-${A3A_REPORT_ARTIFACT_SUFFIX}`; }
...
export async function generatePublishedReportAsset(reportId: string, groupId: string, actor: ReportAssetActor, storage: FileStorage = createFileStorage()) {
  ...
  const bytes = generateA3aReportArtifact(report);
  const key = storageKey(report);
  try {
    await storage.putObject({ key, body: bytes, contentType: A3A_REPORT_ARTIFACT_MIME_TYPE });
    ...
    return await prisma.$transaction(async (tx) => {
      ...
      const asset = await tx.fileAsset.create({ data: { storageKey: key, originalName: filename(report), mimeType: A3A_REPORT_ARTIFACT_MIME_TYPE, sizeBytes: storedBytes.byteLength, sha256, uploadedById: actor.id } });
      ...
      await tx.auditLog.create({ data: { userId: actor.id, action: "REPORT_PUBLISHED_ASSET_GENERATED", entityType: "QuarterlyReport", entityId: report.id, details: { reportId: report.id, reportingGroupId: groupId, revision: report.revision, fileAssetId: asset.id, sha256 } satisfies Prisma.InputJsonObject } });
