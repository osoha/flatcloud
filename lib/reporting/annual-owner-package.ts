...
function proportional(amountCents: number, basisPoints: number) { return Math.round(amountCents * basisPoints / 10_000); }
...
export async function loadAnnualOwnerPackage(user: ReportingUser, input: { ownerId?: string; year: number }) {
  ...
  const loanRows: AnnualLoanRow[] = loans.flatMap((loan) => { const share = loan.property.ownershipMode === "WHOLE_OBJECT" ? loan.property.ownerships.find((row) => row.ownerId === selectedOwner.id)?.shareBasisPoints ?? (loan.property.ownerId === selectedOwner.id ? 10_000 : 0) : 0; if (!share) return []; const snapshot=loan.snapshots[0]; return [{ id:loan.id, propertyId:loan.propertyId, propertyName:loan.property.name, label:loan.label, lender:loan.lender, outstandingPrincipalCents:proportional(snapshot?.outstandingPrincipalCents??loan.outstandingPrincipalCents,share), annualInterestRateBps:snapshot?.annualInterestRateBps??loan.annualInterestRateBps, asOfDate:snapshot?.asOfDate||null, evidence:"Sazba a jistina nejsou dokladem zaplaceného úroku" }]; });
