...
export const MANUAL_BASELINE_SCHEMA_VERSION = 2 as const;
...
const manualV1 = z.object({ source:z.literal("MANUAL_BASELINE"), schemaVersion:z.literal(1), asOfDate:payload.asOfDate, units:units.partial().optional(), rentRoll:rentRoll.partial().optional(), collections:collections.partial().optional(), deposits:deposits.partial().optional(), leases:leases.partial().optional() }).strict();
const manualV2Units = units.partial().extend({ occupancyBps:z.number().int().min(0).max(10000).optional() }).strict();
const manualV2 = z.object({ source:z.literal("MANUAL_BASELINE"), schemaVersion:z.literal(2), asOfDate:payload.asOfDate, units:manualV2Units.optional(), rentRoll:rentRoll.partial().optional(), collections:collections.partial().optional(), deposits:deposits.partial().optional(), leases:leases.partial().optional() }).strict();
export const manualBaselineSnapshotDataSchema = z.union([manualV1, manualV2]).superRefine((value,ctx)=>{const complete=[units.safeParse(value.units),rentRoll.safeParse(value.rentRoll),collections.safeParse(value.collections),deposits.safeParse(value.deposits),leases.safeParse(value.leases)].every(result=>result.success);if(complete)ctx.addIssue({code:"custom",message:"A complete automatic KPI set must use CALCULATED source."})});
export const quarterSnapshotDataSchema = z.union([calculatedSnapshotDataSchema, manualBaselineSnapshotDataSchema]);
