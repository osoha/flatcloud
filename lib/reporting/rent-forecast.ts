...
function eventDrivenPlanAt(row: RentForecastInput, month: Date, asOf: Date, scenario: RentForecastAssumptions) {
  const boundary=endOfMonth(month), firstMonth=new Date(Date.UTC(asOf.getUTCFullYear(),asOf.getUTCMonth(),1));
  let rent=row.currentRentCents, lastEvent=firstMonth, vacancyUntil:Date|null=null;
  const events:Array<{date:Date;kind:"INDEXATION"|"EXPIRY"}>=[];
  if(row.indexationEnabled&&row.nextIndexationAt){for(let d=row.nextIndexationAt,count=0;d<=boundary&&count<200;d=addYears(d,1),count++)events.push({date:d,kind:"INDEXATION"});}
  if(row.effectiveEnd){
    ...
    for(let d=firstExpiry,count=0;d<=boundary&&count<200;d=term?addMonths(d,term):new Date(8640000000000000),count++)events.push({date:d,kind:"EXPIRY"});
  }
  ...
  for(const event of events){
    ...
    if(event.kind==="EXPIRY"&&(scenario.expiryStrategy??"RENEW")==="RELET"){
      ...
      const vacancy=Math.max(0,scenario.relettingVacancyMonths??1);
      vacancyUntil=vacancy?addMonths(new Date(Date.UTC(event.date.getUTCFullYear(),event.date.getUTCMonth(),1)),vacancy):null;
    }else{
  ...
  const vacant=Boolean(vacancyUntil&&month<vacancyUntil&&boundary>=new Date(Date.UTC(vacancyUntil.getUTCFullYear(),vacancyUntil.getUTCMonth()-Math.max(0,scenario.relettingVacancyMonths??1),1)));
  return {rent,vacant};
}
...
export function calculateRentForecastWithAssumptions(rows: RentForecastInput[], asOf: Date, scenarioKey: string, scenario: RentForecastAssumptions, horizonMonths: number, version: 1 | 2 | 3 = 3) {
  ...
  const months = Array.from({ length: horizonMonths }, (_, index) => {
    ...
    const mfProjectedCents = rows.length && rows.every(row => row.mfMarketRentCents != null)
      ? rows.reduce((sum, row) => sum + (version===3 ? marketRentAt(row,month,asOf,scenario)! : Math.round(row.mfMarketRentCents! * (1 + (version === 1 ? 0 : scenario.marketAnnualGrowthBps ?? 0) / 10_000) ** Math.floor(index / 12))), 0) : null;
