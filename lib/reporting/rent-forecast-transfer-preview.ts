...
export function calculateRentForecastTransferPreview(plan: SavedPlan) {
  ...
  const rows = forecast.unitRows.map((row) => {
    const state = row.effectiveEnd && row.effectiveEnd < effectiveAt ? "RENEWAL_REQUIRED" as const : row.finalPlannedCents === row.finalContractualCents ? "NO_CHANGE" as const : "ADDENDUM_REVIEW" as const;
