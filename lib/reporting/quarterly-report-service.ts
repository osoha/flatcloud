...
export async function updateQuarterlyPropertyReportContent(reportId: string, propertyId: string, input: QuarterlyPropertyReportContentInput, actor: QuarterlyReportActor) {
  return serializableTransaction(async (tx) => {
    ...
    const changed = await tx.quarterlyPropertyReport.updateMany({ where: { quarterlyReportId: reportId, propertyId, quarterlyReport: { status: "DRAFT" } }, data: { propertyStatus: content.propertyStatus, managementCommentary: content.managementCommentary, additionalCommentary: content.additionalCommentary, technicalSections: content.technicalSections, valuationRows: content.valuationRows } });
    if (changed.count !== 1) throw new Error("Property report is missing or no longer editable.");
    await audit(tx, actor.id, "REPORT_PROPERTY_CONTENT_UPDATED", report, { propertyId, changedFields: ["propertyStatus", "managementCommentary", "additionalCommentary", "technicalSections", "valuationRows"], technicalSectionCount: content.technicalSections.length, valuationRowCount: content.valuationRows.length, hasAdditionalCommentary: content.additionalCommentary !== null });
    return tx.quarterlyPropertyReport.findUniqueOrThrow({ where: { quarterlyReportId_propertyId: { quarterlyReportId: reportId, propertyId } } });
  });
}
