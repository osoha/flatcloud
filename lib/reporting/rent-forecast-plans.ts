...
export function rentForecastSnapshotFromLiveReport(report: Awaited<ReturnType<typeof loadLiveReport>>) {
  ...
  return { schemaVersion: 1, scope: report.properties.map((property) => ({ propertyId: property.id, propertyName: property.name })).sort((a, b) => a.propertyId.localeCompare(b.propertyId)), mfReferencePeriod: report.mfBenchmark.release ? `Q${report.mfBenchmark.release.marketQuarter} ${report.mfBenchmark.release.marketYear}` : "Nedostupná", rows: serializedRows(rows).sort((a, b) => a.leaseId.localeCompare(b.leaseId)) } satisfies RentForecastPlanSnapshot;
}
