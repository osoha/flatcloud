...
type HistoricalCharge = { debtTreatment?: "CURRENT" | "HISTORICAL" | "EXCLUDED"; debtTreatmentAt?: Date | null; active: boolean; amountCents: number; dueDate: Date; allocations: Allocation[]; securityDepositOffsets?: Offset[]; creditApplications?: Credit[] };
...
export function overdueDebtCentsAsOf(charge: HistoricalCharge, asOf: Date) { const reclassified = (charge.debtTreatment ?? "CURRENT") !== "CURRENT" && charge.debtTreatmentAt && businessDateKey(charge.debtTreatmentAt) <= businessDateKey(asOf); return !reclassified && charge.active && businessDateKey(charge.dueDate) < businessDateKey(asOf) ? outstandingCentsAsOf(charge, asOf) : 0; }
