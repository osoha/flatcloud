...
export function assertAssetDateNotFuture(value: Date, now = new Date()) {
  if (businessDateKey(value) > businessTodayKey(now)) throw new Error("Datum stavu nesmí být v budoucnosti. Budoucí odhad patří do plánu nebo forecastu, ne do potvrzené historie.");
  return value;
}

function safeMoneyNumber(value: number | bigint) {
  const converted = Number(value);
  if (!Number.isSafeInteger(converted)) throw new Error("Finanční částka je mimo bezpečný rozsah reportu.");
  return converted;
}
