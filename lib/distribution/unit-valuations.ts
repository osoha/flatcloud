import {Prisma,UnitValuationSource} from "@prisma/client";
import {canSeeAll} from "@/lib/auth";
import {serializableTransaction} from "@/lib/serializable";

export const unitValuationSources:Record<UnitValuationSource,string>={INTERNAL_COMPARABLES:"Interní srovnání",MARKET_BENCHMARK:"Potvrzený lokální benchmark",EXTERNAL_APPRAISAL:"Externí odhad",OFFER_PRICE:"Nabídková cena",TRANSACTION:"Realizovaná transakce"};
export async function createUnitValuationSnapshot(actor:{id:string;role:string;allProperties?:boolean},propertyId:string,unitId:string,input:{marketValueCents:number;source:UnitValuationSource;valuationDate:Date;reference?:string|null;note?:string|null},client?:Prisma.TransactionClient){
