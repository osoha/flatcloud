...
export function PropertyBudgetComparison({ budgets, costs, year }: { budgets: BudgetInput[]; costs: CostInput[]; year: number }) {
  ...
  return <section className="card col-12 portfolio-table-card" aria-labelledby="budget-comparison-title" data-testid="budget-comparison">
    ...
    <FinancialComparisonChart title={`Rozpočet, objednáno a skutečnost · ${year}`} rows={rows.map(row => ({ label: `${propertyCostKinds[row.kind]} · ${propertyCostCategories[row.category]}${row.remainingCents != null && row.remainingCents < 0 ? " · Překročeno" : ""}`, values: [row.budgetLines ? row.budgetCents : null, row.committedCents, row.actualCents] }))} series={[{ label: "Rozpočet", color: "#8299b7" }, { label: "Objednáno", color: "#dc7c22" }, { label: "Skutečnost", color: "#1769e0" }]}/>
