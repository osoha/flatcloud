...
export function AdminOperationsPanel() {
  ...
  return <>
    ...
    <dialog id="system-panel" ref={dialog} className="system-panel" aria-labelledby="system-panel-title" onClose={() => { setOpen(false); trigger.current?.focus(); }} onClick={event => { if (event.target instanceof Element && event.target.closest("a[href]")) dialog.current?.close(); if (event.target === event.currentTarget) { const rect = event.currentTarget.getBoundingClientRect(); if (event.clientX < rect.left || event.clientX > rect.right || event.clientY < rect.top || event.clientY > rect.bottom) dialog.current?.close(); } }}>
