...
export function AdminOperationsPanel() {
  ...
  return <>
    ...
    <dialog id="system-panel" ref={dialog} className="system-panel" aria-labelledby="system-panel-title" onKeyDown={event => { if (event.key === "Escape") { event.preventDefault(); event.stopPropagation(); dialog.current?.close(); } }} onClose={() => { setOpen(false); trigger.current?.focus(); }} onClick={event => { if (event.target instanceof Element && event.target.closest("a[href]")) dialog.current?.close(); if (event.target === event.currentTarget) { const rect = event.currentTarget.getBoundingClientRect(); if (event.clientX < rect.left || event.clientX > rect.right || event.clientY < rect.top || event.clientY > rect.bottom) dialog.current?.close(); } }}>
      <div className="system-panel-inner">
        ...
        <div className="system-panel-scroll" aria-busy={loading}>
          <div className="system-refresh"><small>{data ? `Aktualizováno ${time(data.asOf)}` : unavailable ? "Aktualizace není dostupná" : "Načítání statistik…"}</small><button className="system-icon-button" type="button" disabled={loading} aria-label="Obnovit statistiky" onClick={() => setRefreshKey(value => value + 1)}><RefreshCw size={16}/></button></div>
