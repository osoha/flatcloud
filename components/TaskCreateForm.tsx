...
export function TaskCreateForm({ properties, initialPropertyId = "", initialLeaseId = "" }: { properties: PropertyOption[]; initialPropertyId?: string; initialLeaseId?: string }) {
  ...
  if (!properties.length) return <div className="card empty-state"><h2>Nemáte nemovitost s právem editace</h2><p>Úkol může založit správce nebo uživatel s právem editace objektu.</p></div>;

  return <form className="card edit-form" action="/api/tasks" method="post" encType="multipart/form-data">
    <div className="form-grid">
      <label className="field"><span>Nemovitost *</span><select name="propertyId" value={propertyId} onChange={(event) => changeProperty(event.target.value)} required><option value="">Vyberte nemovitost</option>{properties.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
      ...
      <label className="field"><span>Odpovědný</span><select key={property?.id || "none"} name="assigneeId" defaultValue={property?.managerId || ""} disabled={!property}><option value="">Nepřiřazen</option>{property?.managers.map((manager) => <option key={manager.id} value={manager.id}>{manager.name}</option>)}</select></label>
