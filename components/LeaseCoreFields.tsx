...
export function LeaseCoreFields({ unitOptions, tenantOptions, defaultUnitId, defaultTenantId, defaultContractNumber, defaultStartDate, defaultEndDate = "", defaultDueDay = 5, defaultRentTiming = "ADVANCE", defaultVariableSymbol = "", defaultTenantBankAccount = "", proposals = {}, contractNumberProposals = {}, ownerAccountsByUnit = {}, tenantAccountsByTenant = {}, showGenerateCharges = false, defaultAutoChargesEnabled = true, defaultIndexationEnabled = false, defaultIndexationPercent = "", showFinancialOnboarding = false, currentBusinessPeriod = "", defaultDeposit = "" }: Props) {
  ...
  return <>
    ...
    {tenantOptions && <label className="field"><span>Hlavní smluvní strana *</span><select name="tenantId" value={tenantId} onChange={(event) => changeTenant(event.target.value)} required>{tenantOptions.map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select><small>Aktuální verze podporuje jednu smluvní stranu. Další osoby na jednotce nejsou spolunájemci; jejich plná podpora je součástí remodelu.</small></label>}
