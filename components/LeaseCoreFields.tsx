...
export function LeaseCoreFields({ unitOptions, tenantOptions, defaultUnitId, defaultTenantId, defaultContractingPartyIds = [], defaultContractNumber, defaultStartDate, defaultEndDate = "", defaultDueDay = 5, defaultRentTiming = "ADVANCE", defaultVariableSymbol = "", defaultTenantBankAccount = "", proposals = {}, contractNumberProposals = {}, ownerAccountsByUnit = {}, tenantAccountsByTenant = {}, showGenerateCharges = false, defaultAutoChargesEnabled = true, defaultIndexationEnabled = false, defaultIndexationPercent = "", showFinancialOnboarding = false, currentBusinessPeriod = "", defaultDeposit = "" }: Props) {
  ...
  return <>
    ...
    {tenantOptions && <fieldset className="field field-full lease-party-picker"><legend>Další smluvní partneři</legend><p>Vyberte osoby, které jsou spolu s hlavní stranou uvedené jako nájemci ve stejné smlouvě. Obyvatele bez smluvní odpovědnosti evidujte zvlášť.</p><div>{tenantOptions.filter(([value]) => value !== tenantId).map(([value, label]) => <label className="checkbox-field" key={value}><input type="checkbox" name="contractingPartyIds" value={value} checked={additionalPartyIds.has(value)} onChange={(event) => setAdditionalPartyIds((current) => { const updated = new Set(current); if (event.target.checked) updated.add(value); else updated.delete(value); return updated; })}/><span>{label}</span></label>)}</div>{tenantOptions.length < 2 && <small>Nejprve založte druhý profil nájemníka; potom jej zde přidáte do stejné smlouvy.</small>}</fieldset>}
