...
type ValuationDraft = Omit<ValuationRow, "amountCents"> & { amountCzk: string };
...
export function QuarterlyPropertyEditorialEditor({ action, propertyStatus, managementCommentary, initialTechnicalSections, initialValuationRows }: { action: string; propertyStatus: string | null; managementCommentary: string | null; initialTechnicalSections: TechnicalSection[]; initialValuationRows: ValuationRow[] }) {
  ...
  const [valuationRows, setValuationRows] = useState<ValuationDraft[]>(initialValuationRows.map(({ amountCents, ...row }) => ({ ...row, amountCzk: moneyInput(amountCents) })));
  return <form className="edit-form" action={action} method="post">
    ...
    <input type="hidden" name="valuationRows" value={JSON.stringify(valuationRows)}/>
    ...
    <div style={{ marginTop: 16 }}><h3>Ocenění</h3>{valuationRows.map((row, index) => <div className="rule-summary" key={index}><div className="form-grid" style={{ flex: 1 }}>
      ...
      <label className="field"><span>Částka (Kč)</span><input type="text" inputMode="decimal" value={row.amountCzk} onChange={(event) => setValuationRows((rows) => rows.map((item, rowIndex) => rowIndex === index ? { ...item, amountCzk: event.target.value } : item))}/></label>
    ...
    </div></div>)}<button type="button" className="secondary" disabled={valuationRows.length >= 40} onClick={() => setValuationRows((rows) => [...rows, { label: "", amountCzk: "", valueLabel: null, note: null }])}>Přidat řádek ocenění</button></div>
