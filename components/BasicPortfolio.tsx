...
import { ArrowRight, Bell, CheckCircle2, House, ListChecks, WalletCards } from "lucide-react";
...
export function BasicPortfolio({ name, period, rows, photos, expected, paid, debt, taskCount, attention, announcementCount, scopeOptions, selection }: {
  ...
  return <div className="page basic-portfolio" data-guide="portfolio">
    ...
    <div className="basic-summary" aria-label="Souhrn portfolia">
      <Link href="#nemovitosti" className="basic-summary-card basic-homes"><span className="basic-summary-icon"><House size={27}/></span><span className="basic-summary-title">Nájmy a lidé</span><strong>{occupied} <span>z {units.length}</span></strong><small>{units.length ? "obsazených jednotek" : "zatím bez jednotek"}</small><span className="basic-summary-link">Moje nemovitosti <ArrowRight size={16}/></span></Link>
      <Link href="/reporty?view=collections" className="basic-summary-card basic-payments"><span className="basic-summary-icon"><WalletCards size={27}/></span><span className="basic-summary-title">Platby · {month}</span><strong>{money(paid)}</strong><small>uhrazeno z {money(expected)} · zbývá {money(remaining)}</small><div className="basic-progress"><span style={{ width: `${expected ? Math.max(0, Math.min(100, Math.round(paid / expected * 100))) : 0}%` }}/></div><span className="basic-summary-link">Přehled plateb <ArrowRight size={16}/></span></Link>
      <Link href="/ukoly" className="basic-summary-card basic-tasks"><span className="basic-summary-icon"><ListChecks size={27}/></span><span className="basic-summary-title">Úkoly a termíny</span><strong>{taskCount}</strong><small>{taskCount === 1 ? "otevřený úkol" : "otevřených úkolů"}</small><span className="basic-summary-link">Otevřít úkoly <ArrowRight size={16}/></span></Link>
    </div>
    ...
    <section className="basic-attention" aria-labelledby="basic-attention-heading">
      ...
      {debt > 0 && <p className="basic-debt-note">Po splatnosti: <strong>{money(debt)}</strong>. Částka zbývající k úhradě za tento měsíc může zahrnovat i platby před termínem.</p>}
      <div className="basic-attention-grid">
        ...
        {attention.slice(0, announcementCount > 0 ? 2 : 3).map((item, index) => <Link className={`basic-attention-item basic-${item.tone}`} href={item.href} key={`${item.href}-${index}`}><span className="basic-attention-icon"><Bell size={21}/></span><span><strong>{item.title}</strong><small>{item.detail}</small></span><ArrowRight size={18}/></Link>)}
        {!announcementCount && !attention.length && <div className="basic-attention-item basic-calm"><span className="basic-attention-icon"><CheckCircle2 size={22}/></span><span><strong>Vše je v pořádku</strong><small>Nic právě nevyžaduje vaši pozornost.</small></span></div>}
      </div>
    </section>
