...
export function DismissibleDetails({ className = "", summary, dialogLabel, children, viewportModal = false }: { className?: string; summary: React.ReactNode; dialogLabel: string; children: React.ReactNode; viewportModal?: boolean }) {
  ...
  function close() { if (detailsRef.current) detailsRef.current.open = false; setOpen(false); summaryRef.current?.focus(); }
  useEffect(() => { setMounted(true); }, []);
  useEffect(() => {
    if (!open) return;
    if (viewportModal) { dialogRef.current?.showModal(); const previous = document.body.style.overflow; document.body.style.overflow = "hidden"; return () => { dialogRef.current?.close(); document.body.style.overflow = previous; }; }
