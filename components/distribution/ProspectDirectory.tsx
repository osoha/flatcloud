...
export function ProspectDirectory({ prospects, properties, filters, today, total }: {
  ...
  return <section className="card portfolio-table-card crm-directory" id="adresar" aria-labelledby="prospect-directory-title">
    ...
    <form key={JSON.stringify(filters)} className="crm-directory-filters" method="get" action="/distribuce/zajemci#adresar" aria-label="Filtry zájemců">
      ...
      <label className="field"><span>Dům</span><select aria-label="Dům" name="propertyId" defaultValue={filters.propertyId}><option value="">Všechny domy</option>{properties.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</select></label>
      <label className="field"><span>Jednotka</span><select aria-label="Jednotka" name="unitId" defaultValue={filters.unitId}><option value="">Všechny jednotky</option>{properties.map(p => <optgroup key={p.id} label={p.name}>{p.units.map(u => <option key={u.id} value={u.id}>{p.name} · {u.label}</option>)}</optgroup>)}</select></label>
      <label className="field"><span>Fáze příležitosti</span><select aria-label="Fáze příležitosti" name="stage" defaultValue={filters.stage}><option value="">Všechny fáze</option>{Object.entries(distributionOpportunityStages).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
      <label className="field"><span>Termín dalšího kroku</span><select aria-label="Termín dalšího kroku" name="due" defaultValue={filters.due}><option value="">Všechny termíny</option>{Object.entries(nextActionFilters).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
    ...
    <div className="table-wrap"><table aria-label="Přehled zájemců a jejich příležitostí">
      ...
      <tbody>{prospects.map(p => <tr key={p.id} data-prospect-id={p.id}>
        ...
        <td><div className="crm-contact-actions"><Link href={`/distribuce/zajemci?prospectId=${encodeURIComponent(p.id)}#nova-prilezitost`}>Přidat příležitost</Link><DismissibleDetails viewportModal summary="Upravit kontakt" dialogLabel={`Upravit kontakt: ${p.name}`}><form className="form-grid" action={`/api/distribution/prospects/${p.id}`} method="post">
          ...
          <label className="field field-full"><span>Poznámka</span><textarea aria-label="Poznámka" name="note" defaultValue={p.note || ""} maxLength={2000} /></label><button className="primary field-full">Uložit kontakt</button>
