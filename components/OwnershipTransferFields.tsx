...
export function OwnershipTransferFields({ currentOwnerId, currentAccountId, payment = false }: { currentOwnerId: string; currentAccountId?: string | null; payment?: boolean }) {
  ...
  return <>
    ...
    {!payment && <label className="field"><span>Původní vlastnictví od</span><input name="previousValidFrom" type="date" max={today} aria-label="Původní vlastnictví od" aria-describedby={`${requestId}-previous-help`}/><small id={`${requestId}-previous-help`}>Doplňte doložený počátek, pokud dosud není potvrzen v historii. Datum založení v aplikaci se za počátek vlastnictví nepovažuje. Budoucí převod potvrďte až v den účinnosti.</small></label>}
