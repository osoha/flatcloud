...
export function QuarterlyReportPropertyWorkspace({ property, candidates, photoCandidates, editable, baseAction, previewHref, previous, next }: { property: QuarterlyPropertyWorkspaceData; candidates: QuarterlySnapshotCandidate[]; photoCandidates: QuarterlyPropertyPhotoCandidate[]; editable: boolean; baseAction: string; previewHref: string; previous?: { id: string; name: string }; next?: { id: string; name: string } }) {
  ...
  return <div className="quarterly-workspace-content"><div className="card quarterly-property-identity"><span className="quarterly-eyebrow">Kvartální report nemovitosti</span><div className="quarterly-property-heading"><div><h2>{property.propertyName}</h2><p>{property.propertyAddress}</p></div><span className="status">{property.propertyStatus ? propertyStatusLabels[property.propertyStatus] : "Stav nevyplněn"}</span></div></div>
    ...
    <div className="card quarterly-preview-placeholder"><div><span className="quarterly-eyebrow">Investorský výstup pro tuto nemovitost</span><h2>Náhled reportu</h2><p>Otevřete skutečný report z připojeného snapshotu, vybraných fotografií a přiřazené designové šablony ve formátu A4 na šířku.</p></div><Link className="button secondary" href={previewHref}>Náhled reportu</Link></div>
