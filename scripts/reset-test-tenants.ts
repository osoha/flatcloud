...
async function main() {
  const tenants = await prisma.tenant.findMany({ where: { id: { in: tenantIds } }, include: { leases: { include: { unit: { include: { property: true } }, occupants: true, paymentItems: true, charges: { include: { allocations: true } }, securityDepositMovements: { select: { bankTransactionId: true } }, notifications: true, tasks: true, meterReadings: true } }, tasks: true } });
  ...
  const transactionIds = [...new Set(tenants.flatMap((tenant) => tenant.leases.flatMap((lease) => [...lease.charges.flatMap((charge) => charge.allocations.map((allocation) => allocation.transactionId)), ...lease.securityDepositMovements.map((movement) => movement.bankTransactionId).filter((id): id is string => Boolean(id))])))];
  ...
  await prisma.$transaction(async (tx) => {
    ...
    for (const transactionId of transactionIds) { const transaction = await tx.bankTransaction.findUnique({ where: { id: transactionId }, select: { status: true, amountCents: true } }); if (transaction && transaction.amountCents >= 0 && transaction.status !== "IGNORED") await tx.bankTransaction.update({ where: { id: transactionId }, data: { status: "UNMATCHED", suggestedLeaseId: null, matchedRuleId: null, matchNote: "Stav bude přepočten po selektivním test resetu." } }); }
    for (const unitId of affectedUnits) await syncUnitOccupancyCache(tx, unitId);
    await tx.auditLog.create({ data: { action: "TEST_TENANTS_RESET", entityType: "Tenant", entityId: tenantIds.join(","), details: { tenantIds, leaseIds, affectedTransactionIds: transactionIds, affectedUnitIds: affectedUnits, statusExamples: ["OVERPAYMENT", "PARTIAL"] } } });
  });
  for (const transactionId of transactionIds) await recomputeTransactionStatus(transactionId);
