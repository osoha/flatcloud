...
import { paymentLeaseOptionLabel, paymentReassignmentEligible } from "../lib/payment-lease-options";
...
check("active lease remains eligible for payment correction",()=>assert.equal(paymentReassignmentEligible({startDate:new Date("2026-01-01"),endDate:null,terminatedOn:null,cancelledAt:null,charges:[]},new Date("2026-09-06")),true));
check("ended lease is eligible only while it has open debt",()=>{const base={startDate:new Date("2025-01-01"),endDate:new Date("2025-12-31"),terminatedOn:null,cancelledAt:null};assert.equal(paymentReassignmentEligible({...base,charges:[]},new Date("2026-09-06")),false);assert.equal(paymentReassignmentEligible({...base,charges:[{active:true,amountCents:100,allocations:[]}]},new Date("2026-09-06")),true)});
check("same-property imported reassign allowed",()=>assert.equal(crossPropertyReassignError("bank","A","A"),null));
check("cross-property manual reassign rejected",()=>assert.match(crossPropertyReassignError("manual","A","B")||"",/nelze přesunout mezi nemovitostmi/));
check("cross-property imported reassign rejected",()=>assert.match(crossPropertyReassignError("bank","A","B")||"",/nelze přesunout mezi nemovitostmi/));
...
const depositIssue=qualityIssues[2];
check("unit-only deposit access respects VIEW and EDIT",()=>{assert.deepEqual(reportingQualityIssueTarget(depositIssue,{role:"USER",unitMemberships:[{unitId:"U1",permission:"VIEW",unit:{propertyId:"A"}}]}),{href:"/smlouvy/L1#kauce",actionLabel:"Zobrazit →",canEdit:false});assert.deepEqual(reportingQualityIssueTarget(depositIssue,{role:"USER",unitMemberships:[{unitId:"U1",permission:"EDIT",unit:{propertyId:"A"}}]}),{href:"/smlouvy/L1#kauce",actionLabel:"Opravit →",canEdit:true})});
...
for(const [name,source,needles] of [
  ["property-scoped picker",picker,["loadEditablePaymentLeases(user,id)","paymentReassignmentEligible","confirmReassign","paymentLeaseOptionLabel"]],
  ["shared lease options",options,["editableUnitWhere(actor,propertyId)","property.name.localeCompare","paymentLeaseOptionLabel","leaseStatusAt(lease,now)===\"ACTIVE\"","outstandingCents(charge)>0"]],
  ["atomic property-scoped service",service,["serializableTransaction","sourcePropertyId","targetPropertyId","propertyId:sourcePropertyId","paymentReassignmentEligible(target)","paymentAllocation.deleteMany","allocateAvailableTransactionToLeaseTx","recomputeTransactionStatusTx","reconcileCollectionTasksAfterPaymentCorrectionTx"]],
  ["payment audit",service,["previousLeaseIds","previousAllocations","newAllocations","previousCounterpartyName","newCounterpartyName","previousVariableSymbol","newVariableSymbol","propertyId:sourcePropertyId"]],
...
const taskQueue=read("app/ukoly/page.tsx"),rowNavigation=read("components/NavigableTableRow.tsx"),leaseDetail=read("app/smlouvy/[leaseId]/page.tsx"),settlementApply=read("app/api/properties/[id]/leases/[leaseId]/settlement/apply/route.ts"),usersPage=read("app/uzivatele/page.tsx"),schema=read("prisma/schema.prisma");
check("task working queue uses full-row navigation",()=>assert.match(taskQueue,/NavigableTableRow href=\{`\/ukoly\/\$\{task\.id\}`\}/));
check("nested property link keeps its own destination",()=>assert.match(taskQueue,/NavigableTableRow[\s\S]*?<Link className="entity-link" href=\{`\/nemovitosti\/\$\{task\.propertyId\}\/prehled`\}/));
check("row navigation supports keyboard activation",()=>{assert.match(rowNavigation,/event\.key !== "Enter" && event\.key !== " "/);assert.match(rowNavigation,/tabIndex=\{0\}/)});
check("interactive controls do not trigger row navigation",()=>{for(const element of ["a","button","input","select","textarea","summary","details","form"])assert.ok(rowNavigation.includes(element),element);assert.match(rowNavigation,/hasInteractiveTarget\(event\.target, event\.currentTarget\)/)});
check("modifier and middle clicks preserve new-tab behavior",()=>{assert.match(rowNavigation,/event\.metaKey \|\| event\.ctrlKey/);assert.match(rowNavigation,/event\.button !== 1/);assert.match(rowNavigation,/window\.open/)});
check("rows without canonical target remain passive",()=>{assert.match(usersPage,/invitations\.map\(\(invitation\) => <tr/);assert.doesNotMatch(leaseDetail,/lease\.credits\.map\(\(credit\)[\s\S]{0,120}NavigableTableRow/)});
check("settlement UI uses overpayment terminology",()=>{assert.doesNotMatch(leaseDetail,/kredit/i);assert.doesNotMatch(settlementApply,/Kredit/);for(const copy of ["Vyúčtování / přeplatky","Původní přeplatek","Započteno","Historie zápočtů","<span>Přeplatek</span>"])assert.ok(leaseDetail.includes(copy),copy)});
check("internal LeaseCredit semantics remain unchanged",()=>{for(const needle of ["model LeaseCredit {","model LeaseCreditApplication {"])assert.ok(schema.includes(needle),needle);for(const needle of ["creditId","leaseCredit.findFirst","leaseCreditApplication.create"])assert.ok(settlementApply.includes(needle),needle)});
check("clickable rows have hover cursor and keyboard focus",()=>assert.match(css,/\.navigable-table-row\{cursor:pointer[\s\S]*\.navigable-table-row:focus-visible/));
check("row navigation has no authorization side effects",()=>assert.doesNotMatch(rowNavigation,/prisma|membership|permission|reportingGroup|requireUser/));
