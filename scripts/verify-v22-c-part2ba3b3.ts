...
import {
  ...
  footerPeriodLabel,
  quarterLabel,
} from "../lib/reporting/pdf/period-labels";
...
async function main() {
  ...
  await check(
    "cover and footer pass precomposed string labels to React-PDF Text",
    () => {
      assert.match(
        renderer,
        /<Text style=\{styles\.coverPeriod\}>\{coverPeriodLabel\(data\)\}<\/Text>/,
      );
  ...
  await check(
    "PDF metadata title continues to use the precomposed quarter label",
    () => {
      assert.match(
        renderer,
        /title=\{`Kvartální report \$\{data\.reportingGroupName\} \$\{quarterLabel\(data\.quarter\)\} \$\{data\.year\}`\}/,
      );
    },
  );
  await check("a real Q3 PDF render has the PDF signature", async () => {
    const { renderQuarterlyReportPdf } = await import(
      "../lib/reporting/pdf/quarterly-report-pdf"
    );
  ...
  await check(
    "A3b.3 follows A3b.2 and precedes later checkpoints in CI",
    () => {
      assert.ok(
