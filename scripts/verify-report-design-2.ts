...
async function main() {
  ...
  await check("DRAFT and read-only primary-photo workspace states exist", () => { assert.match(workspace, /Primární fotografie reportu/); assert.match(workspace, /editable \?/); assert.match(workspace, /Odebrat z reportu/); assert.match(workspace, /Bez popisku/); assert.match(workspace, /Pro tuto nemovitost nejsou v dokumentech dostupné fotografie/); });
  ...
  await check("REPORT-DESIGN-2 is ordered after DESIGN-1 and before build", () => assert.ok(read(".github/workflows/ci.yml").includes("      - run: npm run verify:report-design-1\n      - run: npm run verify:report-design-2\n      - run: npm run build")));
  await runtimeChecks();
