...
async function main() {
  ...
  await check("EDIT can create DRAFT", () => { assert.equal(canEditReportingGroup(member("EDIT"), "group"), true); assert.match(source, /revision: 1, status: "DRAFT"/); });
  ...
  await check("every initial property receives its exact snapshotId", () => assert.match(source, /propertyId: snapshot\.propertyId, snapshotId: snapshot\.id/));
  ...
  const original = { propertyId: "p", snapshotId: "snapshot-3", propertyStatus: "STABILIZED" as const, managementCommentary: "unchanged", technicalSections: { roof: "ok" }, valuationRows: [{ value: 1 }] };
