...
class MemoryStorage implements FileStorage {
  ...
  deleted: string[] = [];
  async putObject(input: PutObjectInput) { this.objects.set(input.key, input.body); }
...
async function main() {
  ...
  await check("no Prisma schema or migration was added", () => { assert.equal(hash("prisma/schema.prisma"), "2f570cdd3e282bdaa70a04ecd2df087f71059e1751bd2303a969304fc1360680"); assert.equal(fs.readdirSync(path.join(root, "prisma/migrations")).filter((name) => /report.design.2.1|direct.*photo/i.test(name)).length, 0); });
  ...
  await check("generic document permissions and deletion remain byte-for-byte unchanged", () => { assert.equal(hash("lib/documents/service.ts"), "0093e9da6259f5a1d3fdf8ce151456046549651ffb34570f5c391fdde27ff6e1"); assert.equal(hash("lib/documents/access.ts"), "935798dfbe0a1963c228b88133ee6558c3182f0afa4f86f7e93b1b25bf98a5ef"); });
  ...
  await check("existing validation storage and image pipeline are reused without copying", () => { assert.match(route, /prepareDocumentFiles\(form\)/); assert.match(service, /storePreparedDocumentBatch/); assert.match(service, /createStoredDocumentsInTransaction/); assert.match(service, /cleanupStoredDocumentBatch/); assert.equal(hash("lib/documents/batch-service.ts"), "e24d5808ff974adc73f1278765b701f54efc9e58b21bff1875eedc442f7f3f44"); });
  ...
  await check("CI orders DESIGN-2.1 after DESIGN-2 and before build", () => assert.ok(read(".github/workflows/ci.yml").includes("      - run: npm run verify:report-design-2\n      - run: npm run verify:report-design-2-1\n      - run: npm run build")));
  await runtimeChecks();
