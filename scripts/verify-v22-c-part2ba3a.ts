...
async function main() {
  ...
  const workspace = read("app/reporty/kvartalni/[groupId]/reporty/[reportId]/page.tsx");
  ...
  await check("only PUBLISHED UI exposes asset controls and only configured ADMIN/SUPER_ADMIN sees generation", () => { assert.match(workspace, /report\.status === "PUBLISHED"[\s\S]*assets\/download/); assert.doesNotMatch(workspace, /report\.status === "(DRAFT|REVIEW)"[\s\S]{0,300}assets\/download/); assert.match(workspace, /publishedAssetId \?[^:]+assets\/download/); assert.match(workspace, /admin && persistentStorageAvailable \?[^:]+assets\/generate/); });
