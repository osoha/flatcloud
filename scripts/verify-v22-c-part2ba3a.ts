...
async function main() {
  ...
  await check("checksum and metadata derive from exact bytes read after storage", () => { assert.match(service, /storedBytes = await storage\.getObject\(key\)/); assert.match(service, /createHash\("sha256"\)\.update\(storedBytes\)/); assert.match(service, /sizeBytes: storedBytes\.byteLength/); });
