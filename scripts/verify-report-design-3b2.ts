...
function main() {
  const renderer = read("components/reporting/quarterly-property/QuarterlyPropertyReportDocument.tsx"), generated = read("components/reporting/ReportDesignGeneratedBackground.tsx"), admin = read("components/reporting/ReportDesignTemplatePreview.tsx"), editorial = read("lib/reporting/editorial-schema.ts"), css = read("app/globals.css"), ci = read(".github/workflows/ci.yml");
  ...
  check("technical renderer still consumes technicalSections", () => assert.match(renderer, /paginateTechnicalSections\(model\.technicalSections\)/));
  check("technical statuses retain exact semantics", () => { for (const status of ["OK", "WATCH", "ACTION", "RISK"]) assert.match(editorial, new RegExp(`"${status}"`)); });
  check("technical presentation has no dashboard card chrome", () => { assert.doesNotMatch(renderer, /qpr-technical-card|qpr-technical-item|qpr-technical-accent/); assert.match(renderer, /qpr-technical-table/); assert.match(css, /\.qpr-technical-table\{[^}]*gap:0[^}]*border-top:1px solid var\(--qpr-primary\)/); });
  check("technical continuation behavior remains deterministic", () => { assert.match(renderer, /splitText\(section\.commentary, 320\)/); assert.match(renderer, /current\.length === 9/); assert.match(renderer, /if \(index > 0\) flush\(\)/); });
  ...
  check("trend renderer remains unchanged while later selection delegates", () => { assert.equal(digest(segment(renderer, "function Trends", "function AdditionalCommentary")), "c2c5ba45e04b337b5119419ee54533521d233a3b883a8e79de06f08f7e379df2"); assert.match(read("lib/reporting/presentation/quarterly-property-presentation-data.ts"), /resolveQuarterlyPropertyTrendSeries/); });
  ...
  check("storage remains protected and Prisma has only the later nullable narrative", () => { assert.equal(hash("lib/storage/google-drive.ts"), "50d0988f0b1215fc3d0ffe13d0ed5cebbc666e31d999799cce58cc8fceb5eb4b"); assert.equal(hash("lib/storage/locations.ts"), "676e036c2fb1202650525e0e43424ae4840d16d476c0c436317d5346f8b2d9f8"); assert.equal(hash("prisma/schema.prisma"), "fd0fc502f50fafe03e3bc65e79c62a64b6124763bce38716806384e253208a41"); });
