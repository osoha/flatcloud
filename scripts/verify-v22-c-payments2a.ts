...
check("Payments1 remains in CI before Payments2A and build", () => { assert.match(packageJson, /"verify:v22-c-payments2a": "tsx scripts\/verify-v22-c-payments2a\.ts"/); assert.ok(ci.includes("      - run: npm run verify:v22-c-part2ba3b3\n      - run: npm run verify:v22-c-payments1\n      - run: npm run verify:v22-c-payments2a\n      - run: npm run build")); });
