...
await check("notifications and collection tasks depend on real charges only",()=>{const notifications=read("lib/rent-notifications.ts"),tasks=read("lib/tasks.ts");assert.match(notifications,/where: \{ charges: \{ some: \{ active: true \} \} \}/);assert.match(tasks,/overdueDebtCents\(charge\)/)});
