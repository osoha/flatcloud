...
check("collapsed sidebar prevents horizontal overflow and uses avatar-only account access",()=>{assert.match(css,/fc-sidebar-collapsed[\s\S]*sidebar\{width:88px;overflow-x:hidden/);assert.match(css,/fc-sidebar-collapsed[\s\S]*logout-form\{display:none\}/)});
console.log(`TAB-NAV-V2.3 verified: ${n} checks.`);
