...
async function main() {
  ...
  await check("Payments1 follows reporting checkpoints and precedes build in CI", () => assert.ok(read(".github/workflows/ci.yml").includes("      - run: npm run verify:v22-c-part2ba3b3\n      - run: npm run verify:v22-c-payments1\n      - run: npm run build")));
