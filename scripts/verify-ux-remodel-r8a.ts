...
async function main() {
  ...
  await check("property access and plan validation protect writes", async () => {
    await assert.rejects(() => createUnitConditionAssessment({ id: admin.id, role: "OWNER" }, external.id, external.units[0].id, { rating: "B_GOOD", investmentUrgency: "MONITOR", estimatedCapexCents: 0, planStatus: "MONITORING", assessedAt: new Date() }), /oprÃ¡vnÄ›nÃ­/);
    await assert.rejects(() => createUnitConditionAssessment(admin, internal.id, internal.units[0].id, { rating: "D_MAJOR_WORK", investmentUrgency: "IMMEDIATE", estimatedCapexCents: 900_000, planStatus: "APPROVED", assessedAt: new Date() }), /cÃ­lovÃ½ termÃ­n/);
  });

  await check("distribution consumes but does not own the technical snapshot", async () => {
    await assert.rejects(() => createUnitDistributionReadiness(admin, internal.id, internal.units[0].id, { distributionReady: true, assessedAt: new Date() }), /Nejprve doplÅˆte technickÃ© hodnocenÃ­/);
  ...
  await check("portfolio and unit UI expose one clean quality workflow", () => {
    ...
    for (const marker of ["Kvalita a technickÃ½ stav portfolia", "vÅ¡echna spravovanÃ¡ aktiva", "PrioritnÃ­ fronta obnovy", "UnitConditionAssessmentForm"]) assert.match(quality, new RegExp(marker));
    for (const marker of ["id=\"kvalita\"", "Kvalita a plÃ¡n obnovy", "Historie hodnocenÃ­"]) assert.match(detail, new RegExp(marker));
    assert.match(distribution, /TechnickÃ½ stav je samostatnÃ½ podklad/);
...
™XY
™L™KÙ›]ÛİYœÛ[ÚÙKœÜXËÈŠKÚİ˜[]H™Y›İŞHH\İšXq#[°ëH1fZ\˜]™[›ÜİXZ°ëHÙ1&Û[°ïH±kØÚÙÊNÂˆ\ÜÙ\›X]Ú
™XY
‹™Ú]X‹İÛÜšÙ›İÜËØÚK[[ŠKİ™\šYN^\™[[Ù[\KÊNÂˆJNÂ‚ˆÛÛœÛÛK›ÙÊV™[[Ù[Hİ±&ñfY[ˆ	ØÚXÚÜßHÛÛ›Û˜
NÂŸB‚›XZ[Š
K™š[˜[J
