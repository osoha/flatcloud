...
async function main() {
  ...
  await check("portfolio and unit UI expose one clean quality workflow", () => {
    ...
    for (const marker of ["Kvalita a technický stav portfolia", "PortfolioScopePicker", "Prioritní fronta obnovy", "UnitConditionAssessmentForm"]) assert.match(quality, new RegExp(marker));
    for (const marker of ["id=\"kvalita\"", "Kvalita a plán obnovy", "Historie hodnocení"]) assert.match(detail, new RegExp(marker));
    assert.match(distribution, /unitConditionRatings/);
