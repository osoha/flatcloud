...
check("data provenance is secondary and collapsible", () => { assert.match(dataPanel, /<details/); assert.match(dataPanel, /Data a datový záznam/); assert.match(dataPanel, /schemaVersion/); assert.match(dataPanel, /calculatorVersion/); });
...
check("editorial and PDF contracts remain unchanged while later manual-baseline schema evolution is allowed", () => { assert.equal(hash("lib/reporting/editorial-schema.ts"), "1e79b34172eddba838e2bf2beb6ca2867f17ce8803b6f5e67a764294333c609c"); assert.match(read("lib/reporting/snapshot-schema.ts"), /manualBaselineSnapshotDataSchema/); assert.equal(hash("lib/reporting/pdf/quarterly-report-pdf.tsx"), "0d3c3f39d33593d7655a555b3b5bee418f55c676106d270eb2b9f48e2b3f8d24"); assert.equal(hash("lib/reporting/pdf/quarterly-report-pdf-data.ts"), "dcca6ef52c3854c225698999aecf9442e49a3bf8cd530bebc2c3a49911f4a86b"); });
// R32: explicitly approved membership boundary; exact hash renewed after adding DB-backed membership/role regression tests.
check("reporting access, workflow, and quality rules remain protected", () => { assert.equal(hash("lib/reporting/backoffice-access.ts"), "2680a7b868cb73d85b7dd3de9287443461f00675c451f0629c29da4bcea3cb01"); assert.equal(hash("lib/reporting/quarterly-quality-gate.ts"), "bee943a48d16afe527c3f9340947821022d98794066134ff7783dea3d2f4fcf1"); const service = read("lib/reporting/quarterly-report-service.ts"); for (const token of ["submitQuarterlyReportForReview", "returnQuarterlyReportToDraft", "publishQuarterlyReport", "createCorrectionRevision"]) assert.ok(service.includes(token)); const design4a=ci.indexOf("npm run verify:report-design-4a"),trends1=ci.indexOf("npm run verify:report-trends-1"),build=ci.indexOf("npm run build");assert.ok(design4a<trends1&&trends1<build); });
