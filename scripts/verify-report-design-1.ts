...
check("Prisma schema and migrations remain unchanged", () => { assert.equal(hash("prisma/schema.prisma"), "e7166a77b36aae60c19561fd818ed806ad7201c9f9d72758e310b738bb6d0f22"); assert.equal(fs.readdirSync(path.join(root, "prisma/migrations"), { withFileTypes: true }).filter((entry) => entry.isDirectory()).length, 27); });
check("editorial, snapshot, and PDF contracts remain unchanged", () => { assert.equal(hash("lib/reporting/editorial-schema.ts"), "ab6e54e8bee77cfc29a7db197a0dcfd379e6884d656202f97dfd255b7c6f2591"); assert.equal(hash("lib/reporting/snapshot-schema.ts"), "560cc086ddd3bc22d464c968ab598cf01b10e669d0df3a1869fd3e9917bf0c00"); assert.equal(hash("lib/reporting/pdf/quarterly-report-pdf.tsx"), "ae22aeb7e1f81b95bb73ec7dae498811bcdbc380a6c2cd3de40e61d3809b24ff"); assert.equal(hash("lib/reporting/pdf/quarterly-report-pdf-data.ts"), "dcca6ef52c3854c225698999aecf9442e49a3bf8cd530bebc2c3a49911f4a86b"); });
check("reporting access, workflow, and quality rules remain unchanged", () => { assert.equal(hash("lib/reporting/backoffice-access.ts"), "fc9dca56b004f44a663f225f03974fca8926dc5520d425640f4339eac5e39ca0"); assert.equal(hash("lib/reporting/quarterly-report-service.ts"), "eb23b029c13a9a0c4dc869f660569504088938f4cb65b2a1b4ef89a5479cbc8b"); assert.equal(hash("lib/reporting/quarterly-quality-gate.ts"), "bee943a48d16afe527c3f9340947821022d98794066134ff7783dea3d2f4fcf1"); assert.ok(ci.includes("      - run: npm run verify:v22-c-inactive-property-notifications\n      - run: npm run verify:report-design-1\n      - run: npm run build")); });
