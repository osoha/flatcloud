...
const detail = read(detailPath), workspace = read(workspacePath), service = read("lib/reporting/quarterly-report-service.ts");
...
async function main() {
  ...
  await check("workspace query scopes the report by reportId and groupId", () => { assert.match(workspace, /where: \{ id: reportId, reportingGroupId: groupId \}/); assert.match(workspace, /if \(!report\) notFound\(\)/); });
  ...
  await check("no publication or correction route/control is exposed", () => { const uiAndRoutes = detail + workspace + mutationRoutes.join("\n"); assert.doesNotMatch(uiAndRoutes, /publishQuarterlyReport|createCorrectionRevision|Publikovat|Opravn(ý|a)|correction/); assert.match(mutationRoutes[3], /action === "submit-review"/); assert.match(mutationRoutes[3], /action === "return-draft"/); });
  ...
  await check("REVIEW return is restricted to ADMIN and SUPER_ADMIN", () => { assert.equal(canAdminReportingBackoffice(effectiveBackofficePermission("OWNER_VIEWER", "EDIT")), false); assert.equal(canAdminReportingBackoffice(effectiveBackofficePermission("OWNER_VIEWER", "ADMIN")), true); assert.match(workspace, /report\.status === "REVIEW" && admin/); });
  await check("PUBLISHED has no mutation controls", () => { assert.doesNotMatch(workspace, /report\.status === "PUBLISHED" &&/); assert.ok((workspace.match(/report\.status === "DRAFT"/g) || []).length >= 2); });
