...
async function main() {
  ...
  await check("PUBLISHED remains outside DRAFT editing controls", () => { assert.ok((workspace.match(/report\.status === "DRAFT"/g) || []).length >= 2); assert.doesNotMatch(workspace, /report\.status === "PUBLISHED"[\s\S]{0,500}(Použít snapshot|Přepočítat snapshot|Uložit shrnutí)/); });
