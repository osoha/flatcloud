...
async function main() {
  ...
  await check("DRAFT shows snapshot recalculation selection and submit controls", () => { assert.match(workspace, /report\.status === "DRAFT"/); assert.match(workspace, /Použít datový záznam/); assert.match(workspace, /Přepočítat datový záznam/); assert.match(workspace, /Odeslat ke kontrole/); });
  ...
  await check("PUBLISHED remains outside DRAFT editing controls", () => { assert.ok((workspace.match(/report\.status === "DRAFT"/g) || []).length >= 2); assert.match(workspace, /editable=\{report\.status === "DRAFT"\}/); assert.match(workspace, /status === "PUBLISHED"/); assert.doesNotMatch(workspace, /status === "PUBLISHED"[\s\S]{0,500}(Použít datový záznam|Přepočítat datový záznam|Uložit shrnutí)/); });
