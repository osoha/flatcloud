...
function main() {
  ...
  check("technicalSections data loading is unchanged", () => { assert.match(technical, /paginateTechnicalSections\(model\.technicalSections\)/); assert.match(read("lib/reporting/presentation/quarterly-property-presentation-data.ts"), /technicalSections: technical\.success \? technical\.data : \[\]/); });
  check("OK WATCH ACTION RISK remain model semantics", () => { for (const status of ["OK", "WATCH", "ACTION", "RISK"]) assert.match(editorial, new RegExp(`"${status}"`)); assert.equal(hash("lib/reporting/editorial-schema.ts"), "1e79b34172eddba838e2bf2beb6ca2867f17ce8803b6f5e67a764294333c609c"); });
  ...
  check("footer is unchanged", () => assert.match(segment(renderer, "function Page", "function Cover"), /<footer className="qpr-footer" style=\{rect\(config\.footer\)\}><span>\{model\.property\.name\}<\/span><span>Q\{model\.report\.quarter\} \{model\.report\.year\}<\/span><\/footer>/));
  check("trends renderer and selection are unchanged", () => { assert.equal(digest(segment(renderer, "function MiniChart", "function Trends")), "235068c5a2fa2cf172479226aed3ada2504db1c8731ee5142125882fb87da063"); assert.equal(digest(segment(renderer, "function Trends", "function AdditionalCommentary")), "c2c5ba45e04b337b5119419ee54533521d233a3b883a8e79de06f08f7e379df2"); });
  ...
  check("storage remains protected and Prisma has no 3B.3 migration", () => { assert.equal(hash("lib/storage/google-drive.ts"), "50d0988f0b1215fc3d0ffe13d0ed5cebbc666e31d999799cce58cc8fceb5eb4b"); assert.equal(hash("lib/storage/locations.ts"), "676e036c2fb1202650525e0e43424ae4840d16d476c0c436317d5346f8b2d9f8"); assert.equal(hash("prisma/schema.prisma"), "fd0fc502f50fafe03e3bc65e79c62a64b6124763bce38716806384e253208a41"); assert.equal(fs.readdirSync(path.join(root, "prisma/migrations")).filter((name) => /3b3|technical.*pptx/i.test(name)).length, 0); });
