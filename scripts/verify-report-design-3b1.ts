...
function main() {
  ...
  check("ASSET background delivery remains unchanged and later trend selection delegates", () => { assert.equal(hash("app/api/reporting-groups/[groupId]/quarterly-reports/[reportId]/presentation/backgrounds/[role]/route.ts"), "722f0e768649e54eb454e897282ec92adbd346e9ec2845ebe889ab2b993a87aa"); const presentationData = read("lib/reporting/presentation/quarterly-property-presentation-data.ts"); assert.match(presentationData, /additionalCommentary: propertyReport\.additionalCommentary/); assert.match(presentationData, /resolveQuarterlyPropertyTrendSeries/); });
