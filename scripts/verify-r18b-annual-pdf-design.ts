...
async function main() {
  ...
  await check("empty and populated states render as intentional pages", async () => {
    ...
    const data: FrozenAnnualReportPdfData = { reportingGroupName: "AGT-INTERNAL", year: 2025, revision: 1, asOfDate: new Date("2025-12-31T12:00:00Z"), founderLetter: null, executiveSummary: null, investmentThesis: null, valueCreationSummary: null, outlook: null, grossAssetValueCents: null, netAssetValueCents: null, debtCents: null, targetPortfolioValueCents: null, realizedExitProceedsCents: null, plannedExitProceedsCents: null, issuedShares: null, treasuryShares: null, sharePriceCents: null, team: [], groupStructure: { parent: { name: "FlatCloud a.s.", type: "holding", ico: "", address: "", leadership: "" }, subsidiaries: [] }, contact: { companyName: "FlatCloud a.s.", registeredAddress: "", officeAddress: "", phone: "", email: "", dataBox: "", boardMembers: "", investmentCommittee: "", confidentialityNotice: "Důvěrné.", investmentDisclaimer: "Nejde o nabídku." }, properties: [{ propertyName: "Moskevská", propertyAddress: "Moskevská 18", openingValueCents: null, currentValueCents: null, targetValueCents: null, realizedExitProceedsCents: null, plannedExitProceedsCents: null, plannedExitYear: null, investmentCase: null, valueCreationNarrative: null, outlook: null, sourceNote: null, mapLatitude: null, mapLongitude: null, mapLabel: null, mapCardSide: "AUTO", mapPhotoDataUrl: null, snapshot: { revision: 1, source: "CALCULATED", schemaVersion: 1, calculatorVersion: "internal", sourceNote: null, fingerprint: "a".repeat(64) } }] };
    const bytes = await renderAnnualReportPdf(data);
    const document = await PDFDocument.load(bytes);
    assert.equal(document.getPageCount(), 10);
