...
test("R33: unit editor without house grant owns meters and private avatar; cross-unit writes rejected",async({page})=>{
 ...
 await page.goto(`/nemovitosti/${property.id}/vzhled?unitId=${own.id}`);await expect(page.getByRole("heading",{name:"Upravit kartu"})).toBeVisible();await expect(page.getByLabel("Avatar objektu / jednotky")).toHaveValue("library");await expect(page.getByRole("option",{name:"Automaticky",exact:true})).toHaveCount(0);
