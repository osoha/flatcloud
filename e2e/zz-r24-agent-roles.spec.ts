...
test.describe("R24 · agentní role a lifecycle", () => {
  ...
  test("technický správce vytvoří označený provozní úkol jen ve svém scope", async ({ page }) => {
    ...
    const picker = page.getByLabel("Kontext úkolu *");
    ...
    await expect(page.getByLabel("Kontext úkolu *")).toHaveValue(propertyId!);
    ...
    await expect(page.getByLabel("Kontext úkolu *")).toHaveValue("");
    await expect(page.getByLabel("Smlouva / nájemník", {exact:true})).toHaveValue("");
    await page.getByLabel("Kontext úkolu *").selectOption(propertyId!);
