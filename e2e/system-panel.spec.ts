...
test("pravý panel: fokus, mobil, tmavý režim, nedostupná data a oznámení pod úkoly", async ({ page }, testInfo) => {
  ...
  await page.screenshot({ animations: "disabled", path: testInfo.outputPath("system-panel-light.png") });
  ...
  await page.screenshot({ animations: "disabled", path: testInfo.outputPath("system-panel-dark.png") });
  ...
  await page.screenshot({ animations: "disabled", path: testInfo.outputPath("system-panel-mobile.png") });
