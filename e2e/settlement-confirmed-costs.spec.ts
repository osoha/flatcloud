...
test('R26D: confirmed costs enter a marked working protocol without posting or duplicate OPEX', async ({page}) => {
  ...
  try {
    ...
    const payload=validateSource({mode:'EXTERNAL_UNIT',kind:'EXTERNAL',vendor:tag,reference:'QA-EXT',from:'2025-01-01',to:'2025-12-31',supplyAmount:'12000',unitId:unit.id,lines:[{...emptySourceLine('2025-01-01','2025-12-31'),amount:'12000',unitId:unit.id,leaseId:lease.id,service:'HEATING',base:'4000',consumptionComponent:'8000',componentsComplete:true}]});
    ...
    await expect(page.getByText('Spotřební: 8000 Kč',{exact:true})).toBeVisible();
    ...
    expect(protocol.actualCostsCents).toBe(1200000);expect(protocol.chargeId).toBeNull();expect(protocol.creditId).toBeNull();
